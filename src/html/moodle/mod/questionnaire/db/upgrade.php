...
function xmldb_questionnaire_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2009040606) {
        // New file type
        $questionnaire_question_type->typeid=11;
        $questionnaire_question_type->type='File';
        $questionnaire_question_type->has_choices='n';
        $questionnaire_question_type->response_table='response_text';
        $result &= insert_record("questionnaire_question_type", $questionnaire_question_type, false);
        
        // Rename public to ispublic column to solve compatibility problems with Oracle
        if ($CFG->dbtype=='mysql'){
...
/*        $table = new XMLDBTable('questionnaire_survey');
        $field = new XMLDBField('PUBLIC');
        $result &= rename_field($table, $field, 'ISPUBLIC');        
        $table = new XMLDBTable('questionnaire_question');
        $field = new XMLDBField('public');
        $result &= rename_field($table, $field, 'ispublic');*/
    }    
