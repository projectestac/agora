...
//*********** FI
                ...
                {
                    //seek rcontent data
                   $rcontent = get_record_sql("SELECT * FROM {$CFG->prefix}rcontent where id = ".$Resultado->ResultadoExtendido->idContenidoLMS, true);
...
//MARSUPIAL ********** ELIMINAT -> It can't be hear
...
//********** FI
	                                ...
	                                $resultid = 0;
	                                if (!$rcont_gradeid = get_record_sql("SELECT DISTINCT id FROM {$CFG->prefix}rcontent_grades" .' '. $select, true))
	                                {
	                                    $instance->timecreated = time();
	                                    $resultid = insert_record('rcontent_grades', $instance);
	                                    if($resultid !== false){
	                                ...
	                                else
	                                {
	                                    ...
	                                    $resultid = update_record('rcontent_grades', $instance);
	                                }
...
// ********** FI
                                    {
                                        ...
                                        if (!$rcont_gradeid = get_record_sql("SELECT id FROM {$CFG->prefix}rcontent_grades_details" .' '. $select, true))
                                        {
                                            $instance->timecreated = time();
                                            $resultid = insert_record('rcontent_grades_details', $instance);
                                        }
                                        else
                                        {
                                            $instance->id = $rcont_gradeid->id;
                                            $resultid = update_record('rcontent_grades_details', $instance);
                                        }
                                    }
...
// 2011.09.27 @mmartinez
    $select = "SELECT DISTINCT c.id AS curseid, u.id AS userid
               ...
               LEFT OUTER JOIN {$CFG->prefix}context cx ON c.id = cx.instanceid
               LEFT OUTER JOIN {$CFG->prefix}role_assignments ra ON cx.id = ra.contextid
               LEFT OUTER JOIN {$CFG->prefix}user u ON ra.userid = u.id
...
// ************ FI
    ...
    return (get_record_sql($select, true) != false);
...
function valid_unit($ResultExt, $book, $rcontent, &$unidad)
{
    ...
    {   
        ...
        {
            $unidad = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books_units where id = {$rcontent->unitid}", true);
        ...
        if (isset($ResultExt->idUnidad))
        {
            ...
            if ($unidad == false)
                $unidad = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books_units where bookid = {$book->id} AND code = '{$ResultExt->idUnidad}'", true);
...
// 2012.03.12 @sarjona                
            $actualizar_unidad = $unidad && ((empty($unidad->name) && !empty($ResultExt->UnidadTitulo))
            || (empty($unidad->sortorder) && !empty($ResultExt->UnidadOrden)));
// *********** ORIGINAL
...
// 2012.03.01 @mmartinez 
                    $publisher = get_record('rcommon_publisher', 'id', $book->publisherid);
...
// 2012.03.01 @mmartinez
                	...
                	if ($unidad == false){
                		$unidad = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books_units where bookid = {$rcontent->bookid} AND code = '{$ResultExt->idUnidad}'", true);
                	}
...
// ************ FI
                ...
                if ($unidad == false)
                {
                    ...
                    $unid = insert_record('rcommon_books_units', $instance);
					log_to_file("wsSeguimiento: function valid_activity - Add new unit ".(($unid)?'OK':'KO (instance:'.serialize($instance).')'));
					
                    //obtiene el registro            
                    $unidad = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books_units where id = ".$unid, true);
                ...
                else
                {
                    ...
                    $up = update_record('rcommon_books_units', $instance);
...
function valid_activity($ResultExt, $book, $rcontent, $unidad, &$actividad)
{
    ...
    {   
        ...
        {
            $actividad = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books_activities WHERE id = ".$rcontent->activityid, true);
...
// 2012.03.01 @mmartinez
                $select = "SELECT * FROM {$CFG->prefix}rcommon_books_activities WHERE unitid = ".$unidad->id." AND code = '".$ResultExt->idActividad."'";
// ********** ORIGINAL
				//$select = "SELECT * FROM {$CFG->prefix}rcommon_books_activities WHERE unitid = ".$unidad->id." AND code = '".$ResultExt->idActividad."'";
// ********** FI
                $actividad = get_record_sql($select, true);
            }
            
            //si existe la unidad pero cambia el orden o el titulo
// MARSUPIAL ************ MODIFICAT -> Just update if no isset unit/actividad title or no isset unit/activity order
// 2012.03.12 @sarjona                
			$actualizar_actividad = $actividad && ((empty($activitat->name) && !empty($ResultExt->ActividadTitulo))
			|| (empty($activitat->sortorder) && !empty($ResultExt->ActividadOrden)));
// *********** ORIGINAL
...
// 2012.03.01 @mmartinez 
                	...
                	if ($actividad == false) {
                    	$actividad = get_record_sql("SELECT id FROM {$CFG->prefix}rcommon_books_activities WHERE unitid = ".$unidad->id." AND code = '".$ResultExt->idActividad."'", true);
                	}
...
// ************ FI
                ...
                if ($actividad == false)
                {
                    ...
                    $add = insert_record('rcommon_books_activities', $instance);
                ...
                else
                {
                    ...
                    $up = update_record('rcommon_books_activities', $instance);
...
function valid_unit_activity($ResultExt, &$unidadid, &$actividadid, &$err)
{
    ...
    try
    {
        ...
        //search rcontent
        $rcontent = get_record_sql("SELECT * FROM {$CFG->prefix}rcontent where id = ".$ResultExt->idContenidoLMS, true);

        //search book
        if ($book = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books where id = {$rcontent->bookid}", true))
        {
...
function UserAuthentication($post_data, $ResultExt)
{
    ...
    try
    {
        ...
        {
            if ($rcontent = get_record_sql("SELECT * FROM {$CFG->prefix}rcontent where id = ".$ResultExt->idContenidoLMS, true))
            {
                if ($book = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books where id = {$rcontent->bookid}", true))
                {
...
//********** MODIFICAT MARSUPIAL - changed validation table to rcommon_publisher
                        if ($track_credent = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_publisher where id = ".$book->publisherid, true))
//**********
                        {
                            $keys = array("Envelope", "Header", "WSEAuthenticateHeader", "User", "value");
