...
function rscorm_add_instance($scorm) {
    global $CFG;
...
// MARSUPIAL ********** ELIMINAT - not by reference just by selects
    /*if (($packagedata = rscorm_check_package($scorm)) != null) {
...
// MARSUPIAL ********** ELIMINAT - not needed becouse save by selects
        /*if (!rscorm_external_link($scorm->reference)) {
            $scorm->md5hash = md5_file($CFG->dataroot.'/'.$scorm->course.'/'.$scorm->reference);
        } else {
            $scorm->dir = $CFG->dataroot.'/'.$scorm->course.'/moddata/rscorm';
...
//********** FI
        ...
        $id = insert_record('rscorm', $scorm);
// MARSUPIAL ********** ELIMINAT - not needed becouse save by selects
        /*if (rscorm_external_link($scorm->reference) || ((basename($scorm->reference) != 'imsmanifest.xml') && ($scorm->reference[0] != '#'))) {
            ...
            if (file_exists($scorm->dir.'/'.$id)) {
                ...
                rscorm_delete_files($scorm->dir.'/'.$id);
            }
...
//********* FI
            $scorm->id = $id;
// MARSUPIAL ********** ELIMINAT - not needed becouse save by selects
            /*$scorm->launch = rscorm_parse($scorm);
            set_field('scorm','launch',$scorm->launch,'id',$scorm->id);
        }*/
//********* FI

        rscorm_grade_item_update(stripslashes_recursive($scorm));
        return $id;
// MARSUPIAL ********** ELIMINAT - not needed becouse save by selects
    /*} else {
        print_error('badpackage','rscorm');
    }*/
//********** FI
}
...
* @return int
*/
function rscorm_update_instance($scorm) {
    global $CFG;
...
// MARSUPIAL ********** ELIMINAT - not needed becouse save by selects
    ...
    if (($packagedata = rscorm_check_package($scorm)) != null) {
        ...
        if ($packagedata->launch == 0) {
            ...
            $scorm->datadir = $packagedata->datadir;
            $scorm->parse = 1;
            if (!rscorm_external_link($scorm->reference) && $scorm->reference[0] != '#') { //dont set md5hash if this is from a repo.
                $scorm->md5hash = md5_file($CFG->dataroot.'/'.$scorm->course.'/'.$scorm->reference);
...
// MARSUPIAL ********** ELIMINAT - not needed becouse save by selects
    ...
    if ($scorm->parse == 1) {
        ...
        if (is_dir($scorm->dir.'/'.$scorm->id)) {
            rscorm_delete_files($scorm->dir.'/'.$scorm->id);
        }
        if (isset($scorm->datadir) && ($scorm->datadir != $scorm->id) && 
           (rscorm_external_link($scorm->reference) || ((basename($scorm->reference) != 'imsmanifest.xml') && ($scorm->reference[0] != '#')))) {
            rename($scorm->dir.$scorm->datadir,$scorm->dir.'/'.$scorm->id);
        }

        $scorm->launch = rscorm_parse($scorm);
    } else {
        $oldscorm = get_record('rscorm','id',$scorm->id);
        $scorm->reference = $oldscorm->reference; // This fix a problem with Firefox when the teacher choose Cancel on overwrite question
    }*/
//********* FI  
    if ($result = update_record('rscorm', $scorm)) {
        rscorm_grade_item_update(stripslashes_recursive($scorm));
        //rscorm_grade_item_update($scorm);  // John Macklins fix - dont think this is needed
    }
...
function rscorm_delete_instance($id) {
    ...
    if (! $scorm = get_record('rscorm', 'id', $id)) {
        return false;
    }

    $result = true;
// MARSUPIAL ********* ELIMINAT - not needed becouse we dont save manifest
    /*$scorm->dir = $CFG->dataroot.'/'.$scorm->course.'/moddata/rscorm';
    if (is_dir($scorm->dir.'/'.$scorm->id)) {
...
//********** FI
    ...
    if (! delete_records('rscorm_scoes_track', 'scormid', $scorm->id)) {
        $result = false;
    }
    if ($scoes = get_records('rscorm_scoes','scorm',$scorm->id)) {
        foreach ($scoes as $sco) {
            if (! delete_records('rscorm_scoes_data', 'scoid', $sco->id)) {
                $result = false;
            }
        } 
// MARSUPIAL ********** AFEGIT - delete data from rscorm_scoes_user to
        if (!delete_records('rscorm_scoes_users','scormid',$id)) {
            $result = false;
        }
//*********** FI

// MARSUPIAL ********** MODIFICAT -> Fixed bug when there is no rscorm_scoes (in this case it's not necessary to return false)
// 2011.11.24 @sarjona
        if (!delete_records('rscorm_scoes','scormid',$id)) {
            $result = false;
        }
// ********** ORIGINAL
/*        
    delete_records('rscorm_scoes', 'scorm', $scorm->id);
    } else {
        $result = false;
 */
// ********* FI
    ...
    if (! delete_records('rscorm', 'id', $scorm->id)) {
        $result = false;
    }
    ...
    rscorm_grade_item_delete(stripslashes_recursive($scorm));
...
function rscorm_user_outline($course, $user, $mod, $scorm) { 
    ...
    $return = rscorm_grade_user($scorm, $user->id, true);
...
function rscorm_user_complete($course, $user, $mod, $scorm) {
    ...
    if ($orgs = get_records_select('rscorm_scoes',"scorm='$scorm->id' AND organization='' AND launch=''",'id','id,identifier,title')) {
        if (count($orgs) <= 1) {
            unset($orgs);
            $orgs[]->identifier = '';
        }
        ...
        foreach ($orgs as $org) {
            ...
            if ($scoes = get_records_select('rscorm_scoes',"scorm='$scorm->id' $organizationsql order by id ASC")){
                    ...
                    if ($sco->launch) {
                        ...
                        if ($usertrack=rscorm_get_tracks($sco->id,$user->id)) {
                            ...
                            if ($sco->scormtype == 'sco') {
                                $report .= '<img src="'.$scormpixdir.'/'.'notattempted.gif" alt="'.get_string('notattempted','rscorm').'" title="'.get_string('notattempted','rscorm').'" />';
                            } else {
                                $report .= '<img src="'.$scormpixdir.'/'.'asset.gif" alt="'.get_string('asset','rscorm').'" title="'.get_string('asset','rscorm').'" />';
                            }
                        }
...
function rscorm_cron () {
    ...
    $timenow = time();
    $updatetime = usergetmidnight($timenow, $sitetimezone) + ($CFG->rscorm_updatetimelast * 3600);
    ...
    if ($CFG->rscorm_updatetimelast < $updatetime and $timenow > $updatetime) {
        ...
        $scormsupdate = get_records('rscorm','updatefreq',RSCORM_UPDATE_EVERYDAY);
        if (!empty($scormsupdate)) {
            foreach($scormsupdate as $scormupdate) {
                $scormupdate->instance = $scormupdate->id;
                $id = rscorm_update_instance($scormupdate);
            }
        }
    }
...
function rscorm_get_user_grades($scorm, $userid=0) {
    ...
    $grades = array();
    if (empty($userid)) {
        if ($scousers = get_records_select('rscorm_scoes_track', "scormid='$scorm->id' GROUP BY userid", "", "userid,null")) {
        ...
        if (!get_records_select('rscorm_scoes_track', "scormid='$scorm->id' AND userid='$userid' GROUP BY userid", "", "userid,null")) {
 ...
 */
function rscorm_update_grades($scorm=null, $userid=0, $nullifnone=true) {
    global $CFG;
    ...
    if ($scorm != null) {
        ...
        $sql = "SELECT s.*, cm.idnumber as cmidnumber
                  FROM {$CFG->prefix}rscorm s, {$CFG->prefix}course_modules cm, {$CFG->prefix}modules m
...
function rscorm_grade_item_update($scorm, $grades=NULL) {
    ...
    if (($scorm->grademethod % 10) == 0) { // RSCORM_GRADESCOES
        if ($maxgrade = count_records_select('rscorm_scoes',"scorm='$scorm->id' AND launch<>'".sql_empty()."'")) {
...
function rscorm_option2text($scorm) {
    $rscorm_popoup_options = rscorm_get_popup_options_array();

    if (isset($scorm->popup)) {
        if ($scorm->popup == 1) {
            $optionlist = array();
            foreach ($rscorm_popoup_options as $name => $option) {
                if (isset($scorm->$name)) {
...
function rscorm_reset_course_form_definition(&$mform) {
    $mform->addElement('header', 'scormheader', get_string('modulenameplural', 'rscorm'));
    $mform->addElement('advcheckbox', 'reset_scorm', get_string('deleteallattempts','rscorm'));
}
...
function rscorm_reset_gradebook($courseid, $type='') {
    ...
    $sql = "SELECT s.*, cm.idnumber as cmidnumber, s.course as courseid
              FROM {$CFG->prefix}rscorm s, {$CFG->prefix}course_modules cm, {$CFG->prefix}modules m
             WHERE m.name='rscorm' AND m.id=cm.module AND cm.instance=s.id AND s.course=$courseid";
...
function rscorm_reset_userdata($data) {
    ...
    if (!empty($data->reset_scorm)) {
        ...
        delete_records_select('rscorm_scoes_track', "scormid IN ($scormssql)");
 ...
 */
function rscorm_is_moddata_trusted() {
    return true;
}
