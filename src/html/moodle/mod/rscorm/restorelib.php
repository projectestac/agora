<?php 
    ...
    //                        |                                                                 |
    //                        |                                                                 |
    //                   scorm_scoes                           scorm_scoes_user                 |
    //              (UL,pk->id, fk->scorm)---------(UL,pk->id,fk->scormid,fk->scoid,fk->userid) |                                                    |
    //                        |                                                                 |
    //                        |                                                                 |
    //                        |                                                                 |
    //                        |                                                                 |
    ...
    function rscorm_restore_mods($mod,$restore) {
...
// MARSUPIAL ********* AFEGIT - added levelid, bookid, unitid, activityid
            ...
            if(!$level=get_record('rcommon_level','code',$levelid)){
            	...
            	notify("Level code: $levelid not found in bd for activity name: $scorm->name!");
            }else{
            	$levelid=$level->id;
            }
            $scorm->levelid = $levelid;
            
            $booktype='isbn';
            $bookid=backup_todb($info['MOD']['#']['BOOKID']['0']['#']);
            if(!$book=get_record('rcommon_books','isbn',$bookid)){
            	//mostramos mensaje de error
            	notify("Book $booktype: $bookid not found in bd for activity name: $scorm->name");
            }else{
            	$booktype='id';
            	$bookid=$book->id;
            }
            ...
            if($unitid!=''){
            	if(!$unit=get_record('rcommon_books_units','bookid',$bookid,'code',$unitid)){
            		//mostramos mensaje de error
            		notify("Unit $unittype: $unitid not found in bd for book $booktype: $bookid on activity name: $scorm->name!");
            	}else{
            		$unittype='id';
            		$unitid=$unit->id;
            	}
            ...
            $scorm->activityid = $activityid;
            
            $scorm->whatgrade = backup_todb($info['MOD']['#']['WHATGRADE']['0']['#']);
//********** FI
            ...
            $newid = insert_record ("rscorm",$scorm);
            //Do some output
            if (!defined('RESTORE_SILENTLY')) {
                echo "<li>".get_string("modulename","rscorm")." \"".format_string(stripslashes($scorm->name),true)."\"</li>";
            }
            backup_flush(300);
            ...
            if ($newid) {
                ...
                $status = rscorm_restore_files ($scorm, $restore);

                if ($status) {
                    $status = rscorm_scoes_restore_mods ($newid,$info,$restore,$mod->id);
// MARSUPIAL ********** ELIMINAT - becouse we have it by user not by sco
                    /*if ($status) {
                        $launchsco = backup_getid($restore->backup_unique_code,"rscorm_scoes",$oldlaunch);
...
//********** FI
    ...
    //This function restores the rscorm_scoes
    function rscorm_scoes_restore_mods($scorm_id,$info,$restore,$oldmodid) {
        ...
        for($i = 0; $i < sizeof($scoes); $i++) {
            ...
            //Now, build the rscorm_scoes record structure
            $sco->scorm = $scorm_id;
...
//********* FI
            ...
            //The structure is equal to the db, so insert the rscorm_scoes
            $newid = insert_record ("rscorm_scoes",$sco);
// MARSUPIAL ********** AFEGIT - add launch by user
        	rscorm_scoes_users_restore_mods ($scorm_id,$newid,$sub_info,$restore);
//********** FI
            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_scoes", $oldid, $newid);
            } else {
                $status = false;
            }
        }
        ...
        rscorm_scoes_seq_objective_restore_mods ($newid,$info,$restore);
        rscorm_scoes_seq_rolluprule_restore_mods ($newid,$info,$restore);
        rscorm_scoes_seq_ruleconds_restore_mods ($newid,$info,$restore);
        if (restore_userdata_selected($restore,'rscorm',$oldmodid)) {
            //Restore rscorm_scoes
            if ($status) {
                if ($restore->backup_version < 2005031300) {
                    $status = rscorm_scoes_tracks_restore_mods_pre15 ($scorm_id,$info,$restore);
                } else {
                    $status = rscorm_scoes_tracks_restore_mods ($scorm_id,$info,$restore);
                }
            }
        }    
...
// MARSUPIAL ********** AFEGIT - function to restore rscorm_scoes_users
    //restore rscorm_scoes_users contents (executed from rscorm_scoes_restore_mods)
    function rscorm_scoes_users_restore_mods ($scorm_id,$sco_id,$info,$restore) {
        ...
        if (!empty($info['#']['LAUNCHS']['0']['#']['LAUNCH'])) {
        	
            $objectives = $info['#']['LAUNCHS']['0']['#']['LAUNCH'];
        }
        ...
        for($i = 0; $i < sizeof($objectives); $i++) {
            ...
            $objective->scormid = $scorm_id;
            $objective->scoid = $sco_id;
            $objective->userid = backup_todb($obj_info['#']['USERID']['0']['#']);
            $objective->launch = backup_todb($obj_info['#']['URL']['0']['#']);
            ...
            $newid = insert_record ("rscorm_scoes_users",$objective);

            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_scoes_users", $oldid, $newid);
            } else {
                $status = false;
            }
        }
...
//********** FI
     ...
     function rscorm_scoes_seq_objective_restore_mods($sco_id,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($objectives); $i++) {
            ...
            //The structure is equal to the db, so insert the forum_discussions
            $newid = insert_record ("rscorm_seq_objective",$objective);

            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_seq_objective", $oldid, $newid);
            } else {
                $status = false;
            }
            ...
            //Now restore the rscorm_seq_mapinfo for each objective

            $status = rscorm_seq_mapinfo_restore_mods ($sco_id,$newid,$obj_info,$restore);
    ...
    function rscorm_scoes_seq_rolluprule_restore_mods($sco_id,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($rolluprules); $i++) {
            ...
            //The structure is equal to the db, so insert the forum_discussions
            $newid = insert_record ("rscorm_seq_rolluprule",$rolluprule);

            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_seq_rolluprule", $oldid, $newid);
            } else {
                $status = false;
            }
            ...
            $status = rscorm_seq_rolluprulecond_restore_mods ($sco_id, $newid,$obj_info,$restore);
     ...
     function rscorm_scoes_seq_ruleconds_restore_mods($sco_id,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($ruleconds); $i++) {
            ...
            //The structure is equal to the db, so insert the forum_discussions
            $newid = insert_record ("rscorm_seq_ruleconds",$rulecond);

            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_seq_ruleconds", $oldid, $newid);
            } else {
                $status = false;
            }
            ...
            $status = rscorm_seq_rulecond_restore_mods ($sco_id, $newid,$obj_info,$restore);
    ...
    function rscorm_scoes_seq_rulecond_restore_mods($sco_id,$rulecondid,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($rulecondsd); $i++) {
            ...
            //The structure is equal to the db, so insert the forum_discussions
            $newid = insert_record ("rscorm_seq_rulecond",$rulecondd);

            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_seq_rulecond", $oldid, $newid);
            } else {
                $status = false;
            }
    ...
    function rscorm_scoes_seq_rolluprulecond_restore_mods($sco_id,$rolluprule,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($rollupruleconds); $i++) {
            ...
            //The structure is equal to the db, so insert the forum_discussions
            $newid = insert_record ("rscorm_seq_rolluprulecond",$rolluprulecond);

            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_seq_rolluprulecond", $oldid, $newid);
            } else {
                $status = false;
            }
    ...
    function rscorm_scoes_seq_mapinfo_restore_mods($sco_id,$objectiveid,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($mapinfos); $i++) {
            ...
            //The structure is equal to the db, so insert the forum_discussions
            $newid = insert_record ("rscorm_seq_mapinfo",$mapinfo);

            if ($newid) {
                //We have the newid, update backup_ids
                backup_putid($restore->backup_unique_code,"rscorm_seq_mapinfo", $oldid, $newid);
            } else {
                $status = false;
            }
    ...
    //This function restores the rscorm_scoes_track
    function rscorm_scoes_tracks_restore_mods($scorm_id,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($scotracks); $i++) {
            ...
            //Now, build the rscorm_scoes_track record structure
            ...
            $sco = backup_getid($restore->backup_unique_code,"rscorm_scoes",$scotrack->scoid);
            if ($sco != NULL) {
                $scotrack->scoid = $sco->new_id;
            }

            $scotrack->timemodified = time();
            //The structure is equal to the db, so insert the rscorm_scoes_track
            $newid = insert_record ("rscorm_scoes_track",$scotrack);
    ...
    //This function restores the rscorm_scoes_track from Moodle 1.4
    function rscorm_scoes_tracks_restore_mods_pre15 ($scorm_id,$info,$restore) {
    ...
    function rscorm_restore_files ($package, $restore) {
        ...
        if ($status) {
            $scorm_path = $moddata_path."/rscorm";
            //Check it exists and create it
        ...
        if ($status) {
            $temp_path = $CFG->dataroot."/temp/backup/".$restore->backup_unique_code.
                         "/moddata/rscorm/".$package->datadir;
            //Check it exists
            if (is_dir($temp_path)) {
                $todo = true;
            }
        ...
        //If todo, we create the neccesary dirs in course moddata/rscorm
        if ($status and $todo) {
    ...
    //rscorm_decode_content_links_caller() function in each module
    //in the restore process
    function rscorm_decode_content_links ($content,$restore) {
        ...
        if ($foundset[0]) {
            ...
            foreach($foundset[2] as $old_id) {
                ...
                if($rec->new_id) {
                    //Now replace it
                    $result= preg_replace($searchstring,$CFG->wwwroot.'/mod/rscorm/index.php?id='.$rec->new_id,$result);
                } else { 
                    //It's a foreign link so leave it as original
                    $result= preg_replace($searchstring,$restore->original_wwwroot.'/mod/rscorm/index.php?id='.$old_id,$result);
                }
            }
        }
        ...
        if ($foundset[0]) {
            ...
            foreach($foundset[2] as $old_id) {
                ...
                if($rec->new_id) {
                    //Now replace it
                    $result= preg_replace($searchstring,$CFG->wwwroot.'/mod/rscorm/view.php?id='.$rec->new_id,$result);
                } else {
                    //It's a foreign link so leave it as original
                    $result= preg_replace($searchstring,$restore->original_wwwroot.'/mod/rscorm/view.php?id='.$old_id,$result);
                }
            }
        }
    ...
    function rscorm_decode_content_links_caller($restore) {
        global $CFG;
        $status = true;

        if ($scorms = get_records_sql ("SELECT s.id, s.summary
                                   FROM {$CFG->prefix}rscorm s
            ...
            foreach ($scorms as $scorm) {
                ...
                if ($result != $content) {
                    ...
                    $status = update_record("rscorm",$scorm);
                    if (debugging()) {
                        if (!defined('RESTORE_SILENTLY')) {
    ...
    function rscorm_restore_logs($restore,$log) {
