...
// This script uses installed report plugins to print quiz reports
    ...
    if (!empty($id)) {
        ...
        if (!empty($b)) {
            if (! $sco = get_record('rscorm_scoes', 'id', $b)) {
...
/// Print the page header
    ...
    if ($action == 'delete' && has_capability('mod/rscorm:deleteresponses',$contextmodule)) {
        if (rscorm_delete_responses($attemptids, $scorm->id)) { //delete responses.
            notify(get_string('scormresponsedeleted', 'rscorm'), 'notifysuccess');
        }
    }
    ...
    if (empty($b)) {
        if (empty($a)) {
            ...
            if ($scousers=get_records_sql($sql)) {
                ...
                $table->head = array();
                $table->width = '100%';
                if (has_capability('mod/rscorm:deleteresponses',$contextmodule)) {
                    $table->head[]  = '&nbsp;';
                    $table->align[] = 'center';
                    $table->wrap[]  = 'nowrap';
                    $table->size[]  = '10';
                ...
                $table->head[]= get_string('attempt','rscorm');
                $table->align[] = 'center';
                $table->wrap[] = 'nowrap';
                $table->size[] = '*';

                $table->head[]= get_string('started','rscorm');
                $table->align[] = 'center';
                $table->wrap[] = 'nowrap';
                $table->size[] = '*';

                $table->head[]= get_string('last','rscorm');
                $table->align[] = 'center';
                $table->wrap[] = 'nowrap';
                $table->size[] = '*';

                $table->head[]= get_string('score','rscorm');
                $table->align[] = 'center';
                $table->wrap[] = 'nowrap';
                $table->size[] = '*';

                foreach($scousers as $scouser){
                    $userdata = rscorm_get_user_data($scouser->userid);
                    $attempt = rscorm_get_last_attempt($scorm->id,$scouser->userid);
                    for ($a = 1; $a<=$attempt; $a++) {
                        $row = array();
                        if (has_capability('mod/rscorm:deleteresponses',$contextmodule)) {
...
//                        $timetracks = get_record_select('rscorm_scoes_track', $select,'min(timemodified) as started, max(timemodified) as last');
                        $timetracks = rscorm_get_sco_runtime($scorm->id, false, $scouser->userid, $a);
...
//                        $row[] = userdate($timetracks->last, get_string('strftimedaydatetime'));
                        ...
                        $row[] = rscorm_grade_user_attempt($scorm, $scouser->userid, $a);
                        $table->data[] = $row;
                    }
                }
                echo '<div id="scormtablecontainer">';
                if (has_capability('mod/rscorm:deleteresponses',$contextmodule)) {
//MARSUPIAL **************** MODIFICAT: PHP_SELF doesn't works in Agora, it needs REQUEST_URI
...
//*********** FI
                ...
                notify(get_string('noactivity', 'rscorm'));
            }
        } else {
            if (!empty($user)) {
                // User SCORM report
                if ($scoes = get_records_select('rscorm_scoes',"scorm='$scorm->id' ORDER BY id")) {
                    if (!empty($userdata)) {
                        ...
                        $table = new stdClass();
                        $table->head = array(get_string('title','rscorm'),
                                             get_string('status','rscorm'),
                                             get_string('time','rscorm'),
                                             get_string('score','rscorm'),
                                             '');
// MARSUPIAL ********** MODIFICAT - Modified to set score header center
                        //$table->align = array('left', 'center','center','right','left');
                        $table->align = array('left', 'center','center','center','left');
//*********** FI
                        $table->wrap = array('nowrap', 'nowrap','nowrap','nowrap','nowrap');
                        $table->width = '80%';
                        $table->size = array('*', '*', '*', '*', '*');
                        foreach ($scoes as $sco) {
// MARSUPIAL ********** AFEGIT - Look in the new rscorm_scoes_users table for the launch url
                            global $USER;
...
//********** FI
        ...
        if (!empty($userdata)) {
            ...
            print_heading('<a href="'.$CFG->wwwroot.'/mod/rscorm/player.php?a='.$scorm->id.'&amp;mode=browse&amp;scoid='.$sco->id.'" target="_new">'.format_string($sco->title).'</a>');
            echo '<div class="mdl-align">'."\n";
            print_user_picture($user, $course->id, $userdata->picture, false, false);
            echo "<a href=\"$CFG->wwwroot/user/view.php?id=$user&amp;course=$course->id\">".
                 "$userdata->firstname $userdata->lastname</a><br />";
            ...
            $table = new stdClass();
            $table->head = array(get_string('identifier','rscorm'),
            ...
            foreach($trackdata as $element => $value) {
                if (substr($element,0,3) == 'cmi') {
                    if (!(in_array ($element, $printedelements))) {
                        $existelements = true;
                        $row = array();
                        $row[] = get_string($element,'rscorm') != '[['.$element.']]' ? get_string($element,'rscorm') : $element;
                        $row[] = s($value);
                        $table->data[] = $row;
                    }
                }
