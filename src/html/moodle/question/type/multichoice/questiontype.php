...
class question_multichoice_qtype extends default_questiontype {
    ...
    */
    ...
    function grade_responses(&$question, &$state, $cmoptions) {
		...
		//XTEC ************ MODIFICAT - To allow negative marks for questions
		//2010.07.07
		$state->raw_grade = min(max((float) $state->raw_grade,-1.0), 1.0) * $question->maxgrade;
		//************ ORIGINAL
        ...
        if (!empty($state->responses)) {
            foreach ($state->responses as $aid =>$rid){
                if (!empty($answers[$rid])) {
					//XTEC ************ MODIFICAT - To solve problem with LaTeX in multiple choice (MDL-12418)
					//2010.07.07 - MDL-12418
