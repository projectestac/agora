...
if (!empty($add)){
	...
	$isbn       = get_record('rcommon_books', 'id', $id);
...
$books = get_records_sql("SELECT rb.id, rb.name as bk_name, rp.name FROM {$CFG->prefix}rcommon_books rb LEFT JOIN {$CFG->prefix}rcommon_publisher rp ON rb.publisherid = rp.id WHERE rb.isbn NOT IN (SELECT isbn FROM {$CFG->prefix}rcommon_user_credentials WHERE euserid = '{$USER->id}')");
...
<form action="addKey.php" method="post" id="frm" name="frm" onsubmit="validate(this); return false;">
	...
	<inpun type="hidden" name="userid" value="'. $USER->id . '">
...
<label for="credential" style="display:inline-block; width:100px;">' . get_string('credential', 'block_my_books') . '</label> <input type="text" id="credential" name="credential" value="' . ($credential != ""? $credential: "") . '">
	<input type="submit" name="add" value="' . get_string('add') . '">
</form></div>';
