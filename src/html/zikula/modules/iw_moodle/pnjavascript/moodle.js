...
function chgUsers(gid){
	var pars = "module=iw_moodle&func=chgUsers&gid=" + gid;
        show_info();
...
function send(valor){
	...
	var i=1;
	if(valor==1){
		f.action="index.php?module=iw_moodle&type=admin&func=enrole";
	}else{
		var count = 0;
		...
		f.action="index.php?module=iw_moodle&type=admin&func=update_enrole";
	}
	if(!error){f.submit();}		
}

function chgCourseState(id){
	var pars = "module=iw_moodle&func=chgCourseState&id=" + id;
	var myAjax = new Ajax.Request("ajax.php", 
	{
		method: 'get', 
		parameters: pars, 
