...
*/
function iw_moodle_ajax_chgUsers($args)
{
	...
	if (empty($groupMembers)) {
		...
		$content = $pnRender->fetch('iw_moodle_admin_ajax.htm');
 ...
 * @return:	Redirect to the user main page
*/
function iw_moodle_ajax_chgCourseState($args)
{
	$dom = ZLanguage::getModuleDomain('iw_moodle');
	if (!SecurityUtil::checkPermission('iw_moodle::', '::', ACCESS_ADMIN)) {
		AjaxUtil::error(DataUtil::formatForDisplayHTML(__('Sorry! No authorization to access this module.', $dom)));
	...
	$changed = pnModFunc('iw_moodle', 'admin', 'activate', array('id' => $id));
	...
	$content1 = $pnRender->fetch('iw_moodle_admin_ajax.htm');
