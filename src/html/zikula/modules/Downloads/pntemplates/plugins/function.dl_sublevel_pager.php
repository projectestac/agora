...
function smarty_function_dl_sublevel_pager($params, &$smarty) 
{
    ...
    if($total > $itemsperpage) 
	{
        ...
        for($x = 0; $x < $total; $x++) {
			...
			{
                $pager .=  "<a href=\"". pnVarPrepForDisplay(pnModURL('Downloads', 'user', 'sublevel', array( 'cid'=>$cid, 'start' => $previous,'sortby'=> $sortby,'cclause'=> $cclause)))."\"><img border=\"0\" src=\"modules/Downloads/pnimages/user/back.png\"  alt=\"PAGER\" /></a>";
            }
            if(!($x % $itemsperpage)) 
			{
                if($x > 0) 
                ...
                if($x == $start) 
				{
					...
					{
                        $pager .=  " | <a href=\"".pnVarPrepForDisplay(pnModURL('Downloads', 'user', 'sublevel', array('cid'=>$cid,'start'=>$x,'sortby'=> $sortby,'cclause'=> $cclause)))."\">$count</a>\n";
                    }
                }
                $count++;
            }
        ...
        if($next < $total) {
            $pager .=  " | <a href=\"".pnVarPrepForDisplay(pnModURL('Downloads', 'user', 'sublevel', array('cid'=>$cid,'start'=>$next,'sortby'=> $sortby,'cclause'=> $cclause)))."\"><img border=\"0\" src=\"modules/Downloads/pnimages/user/forward.png\"  alt=\"PAGER\" /></a>";
        }
