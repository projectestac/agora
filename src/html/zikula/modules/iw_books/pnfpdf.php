...
class PDF extends FPDF
{
	...
	function Header()
	{
        ...
        if(($modinfo['state'] != 3) ||(!is_null(pnModGetVar('iw_books', 'encap')))){
			$imatge = pnModGetVar('iw_main', 'documentRoot').'/'.pnModGetVar('iw_books', 'encap');
			if (file_exists($imatge) && (pnModGetVar('iw_books', 'encap') != '')){
			    $this->Image($imatge,8,6,180);
			}		    
	    }		
		
		//$this->Image(pnModGetVar('iw_books', 'encap'),8,6,180);
		//$dir = pnModGetVar('iw_main', 'documentRoot')."/public/encap.jpg";
		//$dir = pnModGetVar('iw_main', 'documentRoot').$imatge;
		...
		if (pnModGetVar('iw_books', 'marca_aigua') == 1)
		{
			// Fer marca d'aigua
	...
	function Footer()
	{
		...
		$llegenda = pnModAPIFunc('iw_books', 'user', 'llistaplans');
...
//	print_r($args);
	...
	if ($materia == 'TOT'){
	    ...
	    $nommateria  = pnModAPIFunc('iw_books', 'user', 'nommateria', array('codi_mat' => $materia));    
	}
	...
	foreach ($items as $item) {
		...
		$nommateria = utf8_decode(pnModAPIFunc('iw_books', 'user', 'nommateria', array('codi_mat' => $item['codi_mat'])));
		$nivell_abre = pnModAPIFunc('iw_books', 'user', 'reble', array('nivell' => $item['nivell']));
	...
	foreach($amaterials as $row){
		if ($llis_mat == "1"){
			...
			$pdf->SetFont('Arial','B',$mida);
			$nivell_abre = pnModAPIFunc('iw_books', 'user', 'reble', array('nivell' => $row[nivell]));
