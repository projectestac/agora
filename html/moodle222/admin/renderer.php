...
class core_admin_renderer extends plugin_renderer_base {
    ...
    public function plugins_control_panel(plugin_manager $pluginman) {
        ...
        $table_colclasses = array(
            'displayname', 'systemname', 'source', 'version', 'availability', 'settings',
        );
        if (get_protected_agora()) { 
            $table_head[] = get_string('uninstall','core_plugin');
            $table_colclasses[] = 'uninstall';
        }
        $table->head = $table_head;
        $table->colclasses = $table_colclasses;
        //************ ORIGINAL
        /*  
        $table->head = array(
            get_string('displayname', 'core_plugin'),
