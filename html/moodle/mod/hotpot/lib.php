...
function hotpot_add_response(&$attempt, &$question, &$response) {
    //XTEC ************ MODIFICAT - Fixed "ORA-00932: inconsistent datatypes: expected - got CLOB" error
    //2010.11.14 - @jfern343 - http://tracker.moodle.org/browse/MDL-30255
    global $db, $next_url, $CFG;
    //************ ORIGINAL
    //global $db, $next_url;
    //************ FI
    $loopcount = 1;
    ...
    //2010.11.14 - @jfern343 - http://tracker.moodle.org/browse/MDL-30255  
        ...
        if ($CFG->dbfamily == 'oracle') {
            $nametext = 'TO_CHAR(' . $nametext . ')';
        }
        if (!$question->id = get_field('hotpot_questions', 'id', 'hotpot', $attempt->hotpot, 'md5key', $question->md5key, $nametext, $question->name)) {
    //************ ORIGINAL
...
function hotpot_string_id($str) {
    //XTEC ************ Afegit - Fixed "ORA-00932: inconsistent datatypes: expected - got CLOB" error
    //2010.11.14 - @jfern343 - http://tracker.moodle.org/browse/MDL-30255
    global $CFG;
    //************ FI  
    ...
    if (isset($str) && $str<>'') {
        ...
        $stringtext = 'string';
        if ($CFG->dbfamily == 'oracle') {
            $stringtext = 'TO_CHAR(' . $stringtext . ')';
        }
        if (!$id = get_field('hotpot_strings', 'id', 'md5key', $md5key, $stringtext, $str)) {
        //************ ORIGINAL
