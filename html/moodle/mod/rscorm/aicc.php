<?php
    ...
    if (!empty($command) && confirm_sesskey($sessionid)) {
        ...
        if ($sco = scorm_get_sco($scoid, RSCORM_SCO_ONLY)) {
            if (!$scorm = get_record('rscorm','id',$sco->scorm)) {
                error('Invalid script call');
            }
        } else {
            error('Invalid script call');
            ...
            switch ($command) {
                ...
                case 'putparam':
                    if ($status == 'Running') {
                        ...
                        if (!empty($aiccdata) && has_capability('mod/rscorm:savetrack', get_context_instance(CONTEXT_MODULE, $cm->id))) {
                                    ...
                                    if (isset($datamodel[$element])) {
                                        ...
                                        switch ($element) {
                                            case 'cmi.core.lesson_location':
                                                $id = rscorm_insert_track($USER->id, $scorm->id, $sco->id, $attempt, $element, $value);
                                            break;
                                            case 'cmi.core.lesson_status':
                                                ...
                                                if (empty($value) || isset($exites[$value])) {
                                                    ...
                                                    $id = rscorm_insert_track($USER->id, $scorm->id, $sco->id, $attempt, $subelement, $value);
                                                }
                            ...
                            if (($mode == 'browse') && ($initlessonstatus == 'not attempted')){
                                ...
                                $id = rscorm_insert_track($USER->id, $scorm->id, $sco->id, $attempt, 'cmi.core.lesson_status', 'browsed');
                            }
                            if ($mode == 'normal') {
                                if ($lessonstatus == 'completed') {
									if ($sco = scorm_get_sco($scoid)) {
										...
										$id = rscorm_insert_track($USER->id, $scorm->id, $sco->id, $attempt, 'cmi.core.lesson_status', $lessonstatus);
									}
                                }
                            }
                        }
                ...
                case 'exitau':
                    if ($status == 'Running') {
                        if (isset($SESSION->scorm_session_time) && ($SESSION->scorm_session_time != '')) {
                            if ($track = get_record_select('rscorm_scoes_track',"userid='$USER->id' AND scormid='$scorm->id' AND scoid='$sco->id' AND element='cmi.core.total_time'")) {
