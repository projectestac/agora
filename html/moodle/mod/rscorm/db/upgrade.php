...
function xmldb_rscorm_upgrade($oldversion=0) {
    ...
    if ($result && $oldversion < 2006103100) {
        ...
        /// Define table rscorm_scoes_data to be created
        $table = new XMLDBTable('rscorm_scoes_data');

        /// Adding fields to table rscorm_scoes_data
        ...
        /// Adding keys to table rscorm_scoes_data
        $table->addKeyInfo('primary', XMLDB_KEY_PRIMARY, array('id'));

        /// Adding indexes to table rscorm_scoes_data
        $table->addIndexInfo('scoid', XMLDB_INDEX_NOTUNIQUE, array('scoid'));

        /// Launch create table for rscorm_scoes_data
        $result = $result && create_table($table);

        /// The old fields used in rscorm_scoes
        ...
        /// Retrieve old datas
        if ($scorms = get_records('rscorm')) {
            foreach ($scorms as $scorm) {
                if ($olddatas = get_records('rscorm_scoes','scorm', $scorm->id)) {
                    foreach ($olddatas as $olddata) {
                        ...
                        foreach ($fields as $field => $value) {
                            if ($olddata->$field != $value) {
                                ...
                                $id = insert_record('rscorm_scoes_data', $newdata);
                                $result = $result && ($id != 0);
                            }
                        }
                    }
        ...
        $table = new XMLDBTable('rscorm_scoes');
    ...
    /// Define table rscorm_seq_objective to be created
        $table = new XMLDBTable('rscorm_seq_objective');

    /// Adding fields to table rscorm_seq_objective
        $table->addFieldInfo('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null, null, null);
        $table->addFieldInfo('scoid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
    ...
    /// Adding keys to table rscorm_seq_objective
        $table->addKeyInfo('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->addKeyInfo('scorm_objective_uniq', XMLDB_KEY_UNIQUE, array('scoid', 'id'));
        $table->addKeyInfo('scorm_objective_scoid', XMLDB_KEY_FOREIGN, array('scoid'), 'rscorm_scoes', array('id'));

    /// Launch create table for rscorm_seq_objective
        $result = $result && create_table($table);

    /// Define table rscorm_seq_mapinfo to be created
        $table = new XMLDBTable('rscorm_seq_mapinfo');

    /// Adding fields to table rscorm_seq_mapinfo
        $table->addFieldInfo('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null, null, null);
        $table->addFieldInfo('scoid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
        $table->addFieldInfo('objectiveid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
    ...
    /// Adding keys to table rscorm_seq_mapinfo
        $table->addKeyInfo('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->addKeyInfo('scorm_mapinfo_uniq', XMLDB_KEY_UNIQUE, array('scoid', 'id', 'objectiveid'));
        $table->addKeyInfo('scorm_mapinfo_scoid', XMLDB_KEY_FOREIGN, array('scoid'), 'rscorm_scoes', array('id'));
        $table->addKeyInfo('scorm_mapinfo_objectiveid', XMLDB_KEY_FOREIGN, array('objectiveid'), 'rscorm_seq_objective', array('id'));

    /// Launch create table for rscorm_seq_mapinfo
        $result = $result && create_table($table);

    /// Define table rscorm_seq_ruleconds to be created
        $table = new XMLDBTable('rscorm_seq_ruleconds');

    /// Adding fields to table rscorm_seq_ruleconds
        $table->addFieldInfo('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null, null, null);
        $table->addFieldInfo('scoid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
    ...
    /// Adding keys to table rscorm_seq_ruleconds
        $table->addKeyInfo('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->addKeyInfo('scorm_ruleconds_un', XMLDB_KEY_UNIQUE, array('scoid', 'id'));
        $table->addKeyInfo('scorm_ruleconds_scoid', XMLDB_KEY_FOREIGN, array('scoid'), 'rscorm_scoes', array('id'));

    /// Launch create table for rscorm_seq_ruleconds
        $result = $result && create_table($table);

   /// Define table rscorm_seq_rulecond to be created
        $table = new XMLDBTable('rscorm_seq_rulecond');

    /// Adding fields to table rscorm_seq_rulecond
        $table->addFieldInfo('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null, null, null);
        $table->addFieldInfo('scoid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
    ...
    /// Adding keys to table rscorm_seq_rulecond
        $table->addKeyInfo('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->addKeyInfo('scorm_rulecond_uniq', XMLDB_KEY_UNIQUE, array('id', 'scoid', 'ruleconditionsid'));
        $table->addKeyInfo('scorm_rulecond_scoid', XMLDB_KEY_FOREIGN, array('scoid'), 'rscorm_scoes', array('id'));
        $table->addKeyInfo('scorm_rulecond_ruleconditionsid', XMLDB_KEY_FOREIGN, array('ruleconditionsid'), 'rscorm_seq_ruleconds', array('id'));

    /// Launch create table for rscorm_seq_rulecond
        $result = $result && create_table($table);

   /// Define table rscorm_seq_rolluprule to be created
        $table = new XMLDBTable('rscorm_seq_rolluprule');

    /// Adding fields to table rscorm_seq_rolluprule
        $table->addFieldInfo('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null, null, null);
        $table->addFieldInfo('scoid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
    ...
    /// Adding keys to table rscorm_seq_rolluprule
        $table->addKeyInfo('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->addKeyInfo('scorm_rolluprule_uniq', XMLDB_KEY_UNIQUE, array('scoid', 'id'));
        $table->addKeyInfo('scorm_rolluprule_scoid', XMLDB_KEY_FOREIGN, array('scoid'), 'rscorm_scoes', array('id'));

    /// Launch create table for rscorm_seq_rolluprule
        $result = $result && create_table($table);

    /// Define table rscorm_seq_rolluprulecond to be created
        $table = new XMLDBTable('rscorm_seq_rolluprulecond');

    /// Adding fields to table rscorm_seq_rolluprulecond
        $table->addFieldInfo('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null, null, null);
        $table->addFieldInfo('scoid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
    ...
    /// Adding keys to table rscorm_seq_rolluprulecond
        $table->addKeyInfo('primary', XMLDB_KEY_PRIMARY, array('id'));
        $table->addKeyInfo('scorm_rulluprulecond_uniq', XMLDB_KEY_UNIQUE, array('scoid', 'rollupruleid', 'id'));
        $table->addKeyInfo('scorm_rolluprulecond_scoid', XMLDB_KEY_FOREIGN, array('scoid'), 'rscorm_scoes', array('id'));
        $table->addKeyInfo('scorm_rolluprulecond_rolluprule', XMLDB_KEY_FOREIGN, array('rollupruleid'), 'rscorm_seq_rolluprule', array('id'));

    /// Launch create table for rscorm_seq_rolluprulecond
        $result = $result && create_table($table);
    }
            ...
            foreach ($scorms as $scorm) {
                if ($scoes = get_records('rscorm_scoes','scorm',$scorm->id)) {
                    foreach ($scoes as $sco) {
                        if ($tracks = get_records('rscorm_scoes_track','scoid',$sco->id)) {
   ...
   // Adding missing 'whatgrade' field to table scorm
    ...
    if ($result && $oldversion < 2010070952) {
        $table = new XMLDBTable('rscorm');
    	
        /// Adding fields to table rscorm
        $field = new XMLDBField('levelid');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0', null);
        $result = $result && add_field($table, $field);
        $field = new XMLDBField('bookid');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0', null);
        $result = $result && add_field($table, $field);
        $field = new XMLDBField('unitid');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0', null);
        $result = $result && add_field($table, $field);
        $field = new XMLDBField('activityid');
        $field->setAttributes(XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0', null);
        $result = $result && add_field($table, $field);
        $field = new XMLDBField('reference');
        $result = $result && drop_field($table, $field);

        /// Adding table rscorm_scoes_users
        $table = new XMLDBTable('rscorm_scoes_users');
        $table->addFieldInfo('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null, null, null);
        $table->addFieldInfo('scormid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
        $table->addFieldInfo('scoid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
        $table->addFieldInfo('userid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null, null, '0');
        $table->addFieldInfo('launch', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null, null, null);
