...
// older versions to match current functionality
    ...
    if ($oldversion < 2004033000) {
        table_column("rscorm", "", "auto", "integer", "1", "", "0", "NOT NULL", "summary"); 
    }
    if ($oldversion < 2004040900) {
        table_column("rscorm_sco_users", "", "cmi_core_score_raw", "real", "3", "", "0", "NOT NULL", "cmi_core_session_time");
    }
    ...
    if ($oldversion < 2004071900) {
        table_column("rscorm", "", "maxgrade", "real", "3", "", "0", "NOT NULL", "reference");
        table_column("rscorm", "", "grademethod", "integer", "", "", "0", "NOT NULL", "maxgrade");
    }
    ...
    if ($oldversion < 2005031300) {
        ...
        $oldscoes = get_records_select("rscorm_scoes",null,"id ASC");
        ...
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_userdata_idx ON  prefix_rscorm_scoes_track (userid, scormid, scoid);');
    
        $oldtracking = get_records_select('rscorm_sco_users',null,'id ASC');
    ...
    if ($oldversion < 2005040700) {
        //execute_sql('DROP PRIMARY KEY '.$CFG->prefix.'rscorm_scoes_track_pkey;',false);
        execute_sql('DROP INDEX '.$CFG->prefix.'rscorm_scoes_track_userdata_idx;',false);
        execute_sql('DROP INDEX '.$CFG->prefix.'rscorm_scoes_track_userid_key;',false);
        modify_database('','CREATE UNIQUE INDEX prefix_rscorm_scoes_track_track_idx ON  prefix_rscorm_scoes_track (userid, scormid, scoid, element);');
        execute_sql('ALTER TABLE '.$CFG->prefix.'rscorm_scoes_track ADD PRIMARY KEY ("id");',false);
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_scormid_idx ON  prefix_rscorm_scoes_track (scormid);');
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_userid_idx ON  prefix_rscorm_scoes_track (userid);');
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_scoid_idx ON  prefix_rscorm_scoes_track (scoid);');
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_element_idx ON  prefix_rscorm_scoes_track (element);');
    }
    
    if ($oldversion < 2005041500) {
        if ($scorms = get_records_select('rscorm',null,'id ASC')) {
    ...
    if ($oldversion < 2005042700) {
        $trackingdata = get_records_select("rscorm_scoes_track",null,"id ASC");
    ...
    if ($oldversion < 2005042800) {
       table_column("rscorm", "", "browsemode", "integer", "", "", "1", "NOT NULL", "summary");
    }

    if ($oldversion < 2005050800) {
       table_column("rscorm", "", "width", "integer", "", "", "800", "NOT NULL", "auto");
       table_column("rscorm", "", "height", "integer", "", "", "600", "NOT NULL", "width");
    }

     if ($oldversion < 2005052200) {
       table_column("rscorm_scoes_track", "", "timemodified", "integer", "", "", "0", "NOT NULL", "value");
    }
    
    if ($oldversion < 2005052301) { // Mass cleanup of bad upgrade scripts
        execute_sql("DROP INDEX {$CFG->prefix}rscorm_scoes_track_scormid_idx", false);
        execute_sql("DROP INDEX {$CFG->prefix}rscorm_scoes_track_userid_idx", false);
        execute_sql("DROP INDEX {$CFG->prefix}rscorm_scoes_track_scoid_idx", false);
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_scorm_idx ON  prefix_rscorm_scoes_track (scormid);');
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_user_idx ON  prefix_rscorm_scoes_track (userid);');
        modify_database('','CREATE INDEX prefix_rscorm_scoes_track_sco_idx ON  prefix_rscorm_scoes_track (scoid);');
        ...
        execute_sql("DROP INDEX {$CFG->prefix}rscorm_scoes_track_track_idx", false);
        modify_database('','ALTER TABLE ONLY prefix_rscorm_scoes_track DROP CONSTRAINT prefix_rscorm_scoes_track_pkey');
        modify_database('','ALTER TABLE ONLY prefix_rscorm_scoes_track ADD CONSTRAINT prefix_rscorm_scoes_track_pkey PRIMARY KEY (id)');
        modify_database('','ALTER TABLE ONLY prefix_rscorm_scoes_track ADD UNIQUE (userid, scormid, scoid, element)');
        ...
        table_column('rscorm_scoes','datafromlms','datafromlms','VARCHAR','255','','');
        table_column('rscorm_scoes','maxtimeallowed','maxtimeallowed','VARCHAR','19','','');
    }

    if ($oldversion < 2005062700) {
       table_column("rscorm", "", "popup", "integer", "", "", "0", "NOT NULL", "auto");
    }
    
    if ($oldversion < 2005070600) {
        table_column("rscorm", "", "hidetoc", "integer", "", "", "0", "NOT NULL", "browsemode");
        $scorms = get_records_select("rscorm","","id ASC");
        table_column("rscorm", "browsemode", "hidebrowse", "integer", "", "", "0", "NOT NULL", "");
        if (!empty($scorms)) {
            foreach($scorms as $scorm) {
    ...
    if ($oldversion < 2005092500) {
        table_column("rscorm", "", "hidenav", "integer", "", "", "0", "NOT NULL", "hidetoc");
        table_column("rscorm", "", "options", "varchar", "255", "", "", "NOT NULL","popup");
    }
    
    if ($oldversion < 2005092600) {
        table_column("rscorm_scoes_track", "", "attempt", "integer", "", "", "1", "NOT NULL", "scoid"); 
        execute_sql("DROP INDEX {$CFG->prefix}rscorm_scoes_track_scormid_idx", false);
        modify_database('',"CREATE UNIQUE INDEX prefix_rscorm_scoes_track_uk ON prefix_rscorm_scoes_track(userid,scormid,scoid,attempt,element);");
    }

    if ($oldversion < 2005102800) {
        table_column("rscorm", "", "maxattempt", "integer", "", "UNSIGNED", "1", "NOT NULL", "maxgrade"); 
    }


    if ($oldversion < 2006021401) {
       table_column("rscorm", "", "version", "VARCHAR", "9", "", "", "NOT NULL", "reference");
       ...
       modify_database('', 'ALTER TABLE prefix_rscorm_scoes_track DROP CONSTRAINT prefix_rscorm_scoes_track_userid_key1');
       modify_database('', 'ALTER TABLE prefix_rscorm_scoes_track DROP CONSTRAINT prefix_rscorm_scoes_track_userid_key');
       modify_database('', 'ALTER TABLE prefix_rscorm_scoes_track ADD CONSTRAINT prefix_rscorm_scoes_track_userid_key UNIQUE (userid, scormid, scoid, attempt, element)');
    }

    if ($oldversion < 2006102600) {
        table_column("rscorm", "", "skipview", "integer", "", "", "1", "NOT NULL"); 
    }
