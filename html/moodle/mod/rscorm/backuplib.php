<?php 
    ...
    //This is the "graphical" structure of the rscorm mod:
    //
    //                      scorm                                      
    //                   (CL,pk->id)-----------------------------------------------------------
    //                        |                                                               |
    ...
    function rscorm_backup_mods($bf,$preferences) {
        ...
        $scorms = get_records ('rscorm','course',$preferences->backup_course,'id');
        if ($scorms) {
            foreach ($scorms as $scorm) {
                if (backup_mod_selected($preferences,'rscorm',$scorm->id)) {
                    $status = rscorm_backup_one_mod($bf,$preferences,$scorm);
                }
            }
        }
        return $status;
    ...
    function rscorm_backup_one_mod($bf,$preferences,$scorm) {
        $status = true;

        if (is_numeric($scorm)) {
            $scorm = get_record('rscorm','id',$scorm);
        }
        ...
        if($level=get_record('rcommon_level','id',$scorm->levelid)){
        	$levelcode=$level->code;
        }else{
        	notify("Level id: $scorm->levelid not found in bd.");
        }
        if($book=get_record('rcommon_books','id',$scorm->bookid)){
        	$bookisbn=$book->isbn;
        }else{
        	notify("Book id: $scorm->bookid not found in bd.");
        }
        if($scorm->unitid!=0&&$unit=get_record('rcommon_books_units','id',$scorm->unitid)){
        	$unitcode=$unit->code;
        }
        if($scorm->activityid!=0&&$activity=get_record('rcommon_books_activities','id',$scorm->activityid)){
        ...
        fwrite ($bf,full_tag('MODTYPE',4,false,'rscorm'));
        fwrite ($bf,full_tag('NAME',4,false,$scorm->name));
// MARSUPIAL ********* ELIMINAT - Taked out from bd
        //fwrite ($bf,full_tag('REFERENCE',4,false,$scorm->reference));
//********** FI
// MARSUPIAL ********* AFEGIT - added levelid, bookid, unitid, activityid
        ...
        fwrite ($bf,full_tag('WHATGRADE',4,false,$scorm->whatgrade));
//********* FI
        ...
        $status = backup_rscorm_scoes($bf,$preferences,$scorm->id);
        
        //if we've selected to backup users info, then execute backup_rscorm_scoes_track
        if ($status) {
            if (backup_userdata_selected($preferences,'rscorm',$scorm->id)) {
                $status = backup_rscorm_scoes_track($bf,$preferences,$scorm->id);
            }
            $status = backup_rscorm_files_instance($bf,$preferences,$scorm->id);
    ...
    //Backup rscorm_scoes contents (executed from rscorm_backup_mods)
    function backup_rscorm_scoes ($bf,$preferences,$scorm) {

        global $CFG;

        $status = true;

        $scorm_scoes = get_records('rscorm_scoes','scorm',$scorm,'id');
        //If there is scoes
        if ($scorm_scoes) {
            //Write start tag
...
// MARSUPIAL ********** ELIMINAT - take out becouse launch is by user
                /*fwrite ($bf,full_tag('LAUNCH',6,false,$sco->launch));*/
//********** FI
// MARSUPIAL ********** AFEGIT - add launch by user
                $status = backup_rscorm_scoes_users($bf,$preferences,$scorm,$sco->id);
//********* FI
                fwrite ($bf,full_tag('SCORMTYPE',6,false,$sco->scormtype));
                fwrite ($bf,full_tag('TITLE',6,false,$sco->title));
                $status = backup_rscorm_scoes_data($bf,$preferences,$sco->id);
                $status = backup_rscorm_seq_ruleconds($bf,$preferences,$sco->id);
                $status = backup_rscorm_seq_rolluprule($bf,$preferences,$sco->id);
                $status = backup_rscorm_seq_objective($bf,$preferences,$sco->id);
                //End sco
...
// MARSUPIAL ********** AFEGIT - function to backup rscorm_scoes_users
    //Backup rscorm_scoes_users contents (executed from rscorm_backup_rscorm_scoes)
    function backup_rscorm_scoes_users ($bf,$preferences,$scorm,$sco) {
    	global $CFG;

        $status = true;

        $scorm_scoes_users = get_records_sql("SELECT * FROM {$CFG->prefix}rscorm_scoes_users WHERE scormid='".$scorm."' AND scoid='".$sco."'");
        //If there is scoes
        if ($scorm_scoes_users) {
            //Write start tag
            $status =fwrite ($bf,start_tag('LAUNCHS',4,true));
            //Iterate over each sco
            foreach ($scorm_scoes_users as $sco_user) {
            	
                //Start sco
                $status =fwrite ($bf,start_tag('LAUNCH',5,true));
                //Print submission contents
                fwrite ($bf,full_tag('ID',6,false,$sco_user->id));
                fwrite ($bf,full_tag('USERID',6,false,$sco_user->userid));
                fwrite ($bf,full_tag('URL',6,false,$sco_user->launch));
                fwrite ($bf,full_tag('TIMECREATED',6,false,$sco_user->timecreated));
                fwrite ($bf,full_tag('TIMEMODIFIED',6,false,$sco_user->timemodified));
                
                //End sco
                $status =fwrite ($bf,end_tag('LAUNCH',5,true));
            }
            //Write end tag
            $status =fwrite ($bf,end_tag('LAUNCHS',4,true));
        }
        return $status;
    }
//********** FI
   ...
   //Backup rscorm_scoes_data contents (executed from rscorm_backup_rscorm_scoes)
    function backup_rscorm_scoes_data ($bf,$preferences,$sco) {

        global $CFG;

        $status = true;

        $scorm_sco_datas = get_records('rscorm_scoes_data','scoid',$sco,'id');
        //If there is data
        if ($scorm_sco_datas) {
            //Write start tag
   ...
   //Backup rscorm_scoes_track contents (executed from rscorm_backup_mods)
    function backup_rscorm_scoes_track ($bf,$preferences,$scorm) {

        global $CFG;

        $status = true;

        $scorm_scoes_track = get_records('rscorm_scoes_track','scormid',$scorm,'id');
        //If there is track
    ...
    function backup_rscorm_seq_ruleconds ($bf,$preferences,$sco) {

        global $CFG;

        $status = true;

        $scorm_seq_ruleconditions = get_records('rscorm_seq_ruleconds','scoid',$sco,'id');
        ...
        if ($scorm_seq_ruleconditions) {
            ...
            foreach ($scorm_seq_ruleconditions as $seq_rulecondition) {
                ...
                $status = backup_rscorm_seq_rulecond($bf,$preferences,$seq_rulecondition->id);
                //End sco track
    ...
    function backup_rscorm_seq_rulecond ($bf,$preferences,$ruleconditions) {

        global $CFG;

        $status = true;

        $scorm_seq_ruleconditions = get_records('rscorm_seq_rulecond','ruleconditionsid',$ruleconditions,'id');
        //If there is rulecondition
        if ($scorm_seq_ruleconditions) {
            //Write start tag
    ...
    function backup_rscorm_seq_rolluprule ($bf,$preferences,$sco) {

        global $CFG;

        $status = true;

        $scorm_seq_rolluprules = get_records('rscorm_seq_rolluprule','scoid',$sco,'id');
        //If there is rulecondition
        if ($scorm_seq_rolluprules) {
            ...
            foreach ($scorm_seq_rolluprules as $seq_rolluprule) {
                ...
                $status = backup_rscorm_seq_rolluprulecond($bf,$preferences,$seq_rolluprule->id);
                //End sco track
    ...
    function backup_rscorm_seq_rolluprulecond ($bf,$preferences,$rolluprule) {

        global $CFG;

        $status = true;

        $scorm_seq_rollupruleconditions = get_records('rscorm_seq_rolluprulecond','rollupruleid',$rolluprule,'id');
        //If there is rulecondition
    ...
    function backup_rscorm_seq_objective ($bf,$preferences,$sco) {

        global $CFG;

        $status = true;

        $scorm_seq_objectives = get_records('rscorm_seq_objective','scoid',$sco,'id');
        //If there is rulecondition
        if ($scorm_seq_objectives) {
            ...
            foreach ($scorm_seq_objectives as $seq_objective) {
                ...
                $status = backup_rscorm_seq_mapinfo($bf,$preferences,$seq_objective->id);
                //End sco track
    ...
    function backup_rscorm_seq_mapinfo ($bf,$preferences,$objectives) {

        global $CFG;

        $status = true;

        $scorm_seq_objectives = get_records('rscorm_seq_mapinfo','objectiveid',$objectives,'id');
        //If there is rulecondition
        if ($scorm_seq_objectives) {
            //Write start tag
   ...
   function rscorm_check_backup_mods($course,$user_data=false,$backup_unique_code,$instances=null) {
       if (!empty($instances) && is_array($instances) && count($instances)) {
           $info = array();
           foreach ($instances as $id => $instance) {
               $info += rscorm_check_backup_mods_instances($instance,$backup_unique_code);
           }
           return $info;
       }
        //First the course data
        $info[0][0] = get_string('modulenameplural','rscorm');
        ...
        if ($user_data) {
            $info[1][0] = get_string('scoes','rscorm');
            if ($ids = rscorm_scoes_track_ids_by_course ($course)) {
                $info[1][1] = count($ids);
            } else {
                $info[1][1] = 0;
            }
    ...
    function rscorm_check_backup_mods_instances($instance,$backup_unique_code) {
        $info[$instance->id.'0'][0] = $instance->name;
        $info[$instance->id.'0'][1] = '';
        if (!empty($instance->userdata)) {
            $info[$instance->id.'1'][0] = get_string('scoes','rscorm');
            if ($ids = rscorm_scoes_track_ids_by_instance ($instance->id)) {
    ...
    function backup_rscorm_files_instance($bf,$preferences,$instanceid) {
        global $CFG;
        ...
        $status = check_dir_exists($CFG->dataroot.'/temp/backup/'.$preferences->backup_unique_code.'/moddata/rscorm/',true);
        if ($status) {
            //Only if it exists !! Thanks to Daniel Miksik.
            if (is_dir($CFG->dataroot.'/'.$preferences->backup_course.'/'.$CFG->moddata.'/rscorm/'.$instanceid)) {
                $status = backup_copy_file($CFG->dataroot.'/'.$preferences->backup_course.'/'.$CFG->moddata.'/rscorm/'.$instanceid,
                                           $CFG->dataroot.'/temp/backup/'.$preferences->backup_unique_code.'/moddata/rscorm/'.$instanceid);
            }
        }
    ...
    function backup_rscorm_files($bf,$preferences) {
        ...
        if ($status) {
            if (is_dir($CFG->dataroot.'/'.$preferences->backup_course.'/'.$CFG->moddata.'/rscorm')) {
                $handle = opendir($CFG->dataroot.'/'.$preferences->backup_course.'/'.$CFG->moddata.'/rscorm');
                while (false!==($item = readdir($handle))) {
                    if ($item != '.' && $item != '..' && is_dir($CFG->dataroot.'/'.$preferences->backup_course.'/'.$CFG->moddata.'/rscorm/'.$item)
                        && array_key_exists($item,$preferences->mods['rscorm']->instances)
                        && !empty($preferences->mods['rscorm']->instances[$item]->backup)) {
                        $status = backup_copy_file($CFG->dataroot.'/'.$preferences->backup_course.'/'.$CFG->moddata.'/rscorm/'.$item,
                                                   $CFG->dataroot.'/temp/backup/'.$preferences->backup_unique_code.'/moddata/rscorm/',$item);
                    }
                }
            }
        }
    ...
    function rscorm_encode_content_links ($content,$preferences) {
        ...
        $buscar="/(".$base."\/mod\/rscorm\/index.php\?id\=)([0-9]+)/";
        $result= preg_replace($buscar,'$@SCORMINDEX*$2@$',$content);

        //Link to scorm view by moduleid
        $buscar="/(".$base."\/mod\/rscorm\/view.php\?id\=)([0-9]+)/";
    ...
    function rscorm_ids ($course) {

        global $CFG;

        return get_records_sql ("SELECT a.id, a.course
                                 FROM {$CFG->prefix}rscorm a
                                 WHERE a.course = '$course'");
    }
   
    //Returns an array of rscorm_scoes id
    function rscorm_scoes_track_ids_by_course ($course) {

        global $CFG;

        return get_records_sql ("SELECT s.id , s.scormid
                                 FROM {$CFG->prefix}rscorm_scoes_track s,
                                      {$CFG->prefix}rscorm a
    ...
    //Returns an array of rscorm_scoes id
    function rscorm_scoes_track_ids_by_instance ($instanceid) {
