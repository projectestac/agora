...
function rscorm_seq_navigation ($scoid,$userid,$request,$attempt=0) {
    ...
    switch ($request) {
        ...
        case 'resumeall_':
            if (empty($seq->currentactivity)) {
                if ($track = get_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'element','suspendedactivity')) {//I think it's suspend instead of suspendedactivity
                    $seq->navigation = true;
        ...
        default: /// {target=<STRING>}choice 
            if ($targetsco = get_record('rscorm_scoes','scorm',$sco->scorm,'identifier',$request)) {
                if ($targetsco->parent != '/') {
                    $seq->target = $request;
                } else {
...
function rscorm_seq_end_attempt($sco,$userid,$seq) {
        ...
        if (!isset($sco->tracked) || ($sco->tracked == 1)) {
            if (!rscorm_seq_is('suspended',$sco->id,$userid)) {
                ...
                if (!isset($sco->objectivesetbycontent) || ($sco->objectivesetbycontent == 0)) {
                    if ($objectives = get_records('rscorm_seq_objective','scoid',$sco->id)) {
    ...
    rscorm_seq_overall_rollup($sco,$userid);
}
...
function rscorm_seq_is($what, $scoid, $userid, $attempt=0) {
    ...
    $active = false;
    if ($track = get_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'attempt',$attempt,'element',$what)) {
        $active = true;
    }
    return $active;
}
...
function rscorm_seq_set($what, $scoid, $userid, $attempt=0, $value='true') {
    ...
    if ($value == false) {
        delete_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'attempt',$attempt,'element',$what);
    } else {
        rscorm_insert_track($userid, $sco->scorm, $sco->id, 0, $what, $value);
    }

    // update grades in gradebook
    $scorm = get_record('rscorm', 'id', $sco->scorm);
...
function rscorm_seq_rules_check ($sco, $action){
	$act = null;
	if($rules = get_records('rscorm_seq_ruleconds','scoid',$sco->id,'action',$action)){
...
function rscorm_seq_rule_check ($sco, $rule){
	$bag = Array();
	$cond = '';
	$ruleconds = get_records('rscorm_seq_rulecond','scoid',$sco->id,'ruleconditionsid',$rule->id);
...
function rscorm_seq_overall_rollup($sco,$userid){//Carlos
	 ...
	 if ($ancestors = scorm_get_ancestors($sco)) { 
            foreach ($ancestors as $ancestor) {
				...
				rscorm_seq_activity_progress_rollup($sco,$userid);
...
function rscorm_seq_measure_rollup($sco,$userid){
	...
	$objectives = get_records('rscorm_seq_objective','scoid',$sco->id);
	...
	if ($targetobjective != null){
        ...
        foreach ($children as $child){
			...
			if (!isset($child->tracked) || ($child->tracked == 1)){
				...
				$objectives = get_records('rscorm_seq_objective','scoid',$child->id);
                foreach ($objective as $objective){
		            if ($objective->primaryobj == true){//Objective contributes to rollup I'm using primaryobj field, but not 
				...
				if ($rolledupobjective != null){
					...
					if (!rscorm_seq_is('objectivemeasurestatus',$sco->id,$userid)) {
						$normalizedmeasure = get_record('rscorm_scoes_track','scoid',$child->id,'userid',$userid,'element','objectivenormalizedmeasure');
...
/*
	if($targetobjective->satisfiedbymeasure){
		...
		if ((rscorm_seq_rollup_rule_check($sco,$userid,'incomplete'))|| (rscorm_seq_rollup_rule_check($sco,$userid,'completed'))){
            ...
            $rolluprules = get_record('rscorm_seq_rolluprule','scoid',$sco->id,'userid',$userid);
            foreach($rolluprules as $rolluprule){
                $rollupruleconds = get_records('rscorm_seq_rolluprulecond','rollupruleid',$rolluprule->id);
	...
	if ($targetobjective != null){
		...
		if($targetobjective->satisfiedbymeasure){
            ...
            if (!rscorm_seq_is('objectiveprogressstatus',$sco->id,$userid)) {
				...
				$normalizedmeasure = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','objectivenormalizedmeasure');
...
function rscorm_seq_activity_progress_rollup ($sco, $userid){
...
function rscorm_seq_rollup_rule_check ($sco,$userid,$action){
	 ...
	 if($rolluprules = get_record('rscorm_seq_rolluprule','scoid',$sco->id,'userid',$userid,'action',$action)){
		...
		foreach($rolluprules as $rolluprule){
			...
			foreach ($children as $child){
				...
				/*$tracked = get_records('rscorm_scoes_track','scoid',$child->id,'userid',$userid);
...
function rscorm_evaluate_condition ($rolluprulecond,$sco,$userid){
            ...
            switch ($rolluprulecond->cond){
				...
				case 'satisfied':
					 if($r=get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','objectivesatisfiedstatus')){
						if($r->value == true){
					        if ($r=get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','objectiveprogressstatus')){
						        if($r->value == true){
					               $res= true;
						        }
				            }
				...
				case 'objectiveMeasureKnown':
					if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','objectivemeasurestatus')){
					    if($r->value == true){
							$res = true;
						}
				...
				case 'completed':
					if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','attemptcompletionstatus')){
					    if($r->value){
							if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','attemptprogressstatus')){
					           if($r->value){
							      $res = true;
						       }
				...
				case 'attempted':
					if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','activityprogressstatus')){
					    if($r->value){
							if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','activityattemptcount')){
					            if($r->value > 0){
							        $res = true;
						        }
				...
				case 'attemptLimitExceeded':
					if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','activityprogressstatus')){
					    if($r->value){
							if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','limitconditionattemptlimitcontrol')){
					            if($r->value){
							       if ($r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','activityattemptcount') && $r2 = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','limitconditionattemptlimit') ){
					                   if($r->value >= $r2->value){
							               $res = true;
						               }
...
function rscorm_seq_check_child ($sco, $action, $userid){
	$included = false;
	$sco=scorm_get_sco($sco->id);
	$r = get_record('rscorm_scoes_track','scoid',$sco->id,'userid',$userid,'element','activityattemptcount');
...
function rscorm_seq_sequencing ($scoid,$userid,$seq) {
    ...
    switch ($seq->sequencing) {
        ...
        case 'start':
			 $seq = rscorm_seq_start_sequencing($sco,$userid,$seq); //We'll see the parameters we have to send, this should update delivery and end
			$seq->sequencing = true;
        ...
        case 'exit':
			 $seq = rscorm_seq_exit_sequencing($sco,$userid,$seq); //We'll see the parameters we have to send, this should update delivery and end
			$seq->sequencing = true;
        ...
        case 'retry':
            $seq = rscorm_seq_retry_sequencing($sco,$userid,$seq); //We'll see the parameters we have to send, this should update delivery and end
            $seq->sequencing = true;
		...
		case 'previous':
			$seq = rscorm_seq_previous_sequencing($sco,$userid,$seq);// We'll see the parameters we have to send, this should update delivery and end
			$seq->sequencing = true;
		...
		case 'choice':
			$seq = rscorm_seq_choice_sequencing($sco,$userid,$seq);// We'll see the parameters we have to send, this should update delivery and end
             $seq->sequencing = true; 
...
function rscorm_seq_start_sequencing($scoid,$userid,$seq){
	...
	$sco = get_record('rscorm_scoes','scoid',$scoid,'userid',$userid);
...
function rscorm_seq_resume_all_sequencing($scoid,$userid,$seq){
	...
	$track = get_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'element','suspendedactivity');
    if (!$track) {
		$seq->delivery = null;
	    $seq->exception = 'SB.2.6-2';
	    return $seq;
	}
	$seq->delivery = get_record('rscorm_scoes','scoid',$scoid,'userid',$userid);//we assign the sco to the delivery
...
function rscorm_seq_flow_activity_traversal ($activity, $userid, $direction, $childrenflag, $prevdirection, $seq,$userid){//returns the next activity on the tree, traversal direction, control returned to the LTS, (may) exception
   ...
   if (!isset($parent->flow) || ($parent->flow == false)) {
    ...
    $rulch = rscorm_seq_rules_check($sco, 'skipped');
	if ($rulch != null){
	...
	if($direction = 'backward'){
		 ...
		 if (scorm_is_leaf ($activity) || !$childrenflag){
			 ...
			 if ($children[0]->id == $activity->id){
				$seq = rscorm_seq_flow_tree_traversal ($parent, 'backward', false, null, $seq);
				return $seq;
			 }
			 else{
				$ancestors = scorm_get_ancestors($activity);
...
function rscorm_limit_cond_check ($activity,$userid){
    ...
    if (!isset($activity->limitcontrol) || ($activity->limitcontrol == 1)){	
		$r = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','activityattemptcount');
	 ...
	 if (!isset($activity->limitabsdurcontrol) || ($activity->limitabsdurcontrol == 1)){	
		$r = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','activityabsoluteduration');
	...
	if (!isset($activity->limitexpdurcontrol) || ($activity->limitexpdurcontrol == 1)){	
		$r = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','activityexperiencedduration');
	 ...
	 if (!isset($activity->limitattabsdurcontrol) || ($activity->limitattabsdurcontrol == 1)){	
		$r = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','attemptabsoluteduration');
	...
	if (!isset($activity->limitattexpdurcontrol) || ($activity->limitattexpdurcontrol == 1)){	
		$r = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','attemptexperiencedduration');
	...
	if (!isset($activity->limitbegincontrol) || ($activity->limitbegincontrol == 1)){	
		$r = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','begintime');
...
function rscorm_content_delivery_environment ($seq,$userid){
	...
	$track = get_record('rscorm_scoes_track','scoid',$act->id,'userid',$userid,'element','suspendedactivity');
	if ($track != null){
		$seq = rscorm_clear_suspended_activity($seq->delivery, $seq);
		...
		if(!rscorm_seq_is('active',$activity->id,$userid)){
			if(!isset($activity->tracked) || ($activity->tracked == 1)){
				if(!rscorm_seq_is('suspended',$activity->id,$userid)){
					$r = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','activityattemptcount');
					$r->value = ($r->value)+1;
					update_record ('rscorm_scoes_track',$r);
					if ($r->value == 1){
    ...
    if(isset($activity->tracked) || ($activity->tracked == 0)){
		...
		$atabsdur = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','attemptabsoluteduration');
		$atexpdur = get_record('rscorm_scoes_track','scoid',$activity->id,'userid',$userid,'element','attemptexperiencedduration');
	}
	return $seq;

 
}
function rscorm_clear_suspended_activity($act,$seq){
	$currentact= $seq->currentactivity;
	$track = get_record('rscorm_scoes_track','scoid',$currentact->id,'userid',$userid,'element','suspendedactivity');
	if ($track != null){
...
function rscorm_select_children_process($scoid,$userid){
    ...
    if (!scorm_is_leaf($sco)){
		if(!rscorm_seq_is('suspended',$scoid,$userid) && !rscorm_seq_is('active',$scoid,$userid)){
			$r = get_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'element','selectiontiming');
                ...
                case 'once':
                    if(!rscorm_seq_is('activityprogressstatus',$scoid,$userid)){
					    if(rscorm_seq_is('selectioncountsstatus',$scoid,$userid)){
					        $childlist = '';
							$res = get_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'element','selectioncount');
							$i = ($res->value)-1;
							$children = scorm_get_children ($sco);
...
function rscorm_randomize_children_process($scoid,$userid){
    ...
    if (!scorm_is_leaf($sco)){
		if(!rscorm_seq_is('suspended',$scoid,$userid) && !rscorm_seq_is('active',$scoid,$userid)){
			$r = get_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'element','randomizationtiming');
