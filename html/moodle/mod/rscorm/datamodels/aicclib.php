...
* @return mixed
*/
function rscorm_get_aicc_columns($row,$mastername='system_id') {
...
* @return string
*/
function rscorm_forge_cols_regexp($columns,$remodule='(".*")?,') {
    $regexp = '/^';
...
function rscorm_parse_aicc($pkgdir,$scormid) {
    $version = 'AICC';
    $ids = array();
    $courses = array();
    ...
    foreach ($ids as $courseid => $id) {
        ...
        if (isset($id->des)) {
            ...
            $columns = rscorm_get_aicc_columns($rows[0]);
            $regexp = rscorm_forge_cols_regexp($columns->columns);
        ...
        if (isset($id->cst)) {
            ...
            $columns = rscorm_get_aicc_columns($rows[0],'block');
            $regexp = rscorm_forge_cols_regexp($columns->columns,'(.+)?,');
            for ($i=1;$i<count($rows);$i++) {
                if (preg_match($regexp,$rows[$i],$matches)) {
                    for ($j=0;$j<count($columns->columns);$j++) {
        ...
        if (isset($id->ort)) {
            ...
            $columns = rscorm_get_aicc_columns($rows[0],'course_element');
            $regexp = rscorm_forge_cols_regexp($columns->columns,'(.+)?,');
            for ($i=1;$i<count($rows);$i++) {
                if (preg_match($regexp,$rows[$i],$matches)) {
        ...
        if (isset($id->pre)) {
            ...
            $columns = rscorm_get_aicc_columns($rows[0],'structure_element');
            $regexp = rscorm_forge_cols_regexp($columns->columns,'(.+),');
            for ($i=1;$i<count($rows);$i++) {
                if (preg_match($regexp,$rows[$i],$matches)) {
    ...
    $oldscoes = get_records('rscorm_scoes','scorm',$scormid);
    ...
    if (isset($courses)) {
        foreach ($courses as $course) {
            ...
            //print_r($sco);
            if (get_record('rscorm_scoes','scorm',$scormid,'identifier',$sco->identifier)) {
                $id = update_record('rscorm_scoes',$sco);
                unset($oldscoes[$id]);
            } else {
                $id = insert_record('rscorm_scoes',$sco);
            }
            ...
            if (isset($course->elements)) {
                foreach($course->elements as $element) {
                    ...
                    if (isset($element->file_name)) {
                        ...
                        $sco->next = 0;
                        $id = null;
                        if ($oldscoid = rscorm_array_search('identifier',$sco->identifier,$oldscoes)) {
                            $sco->id = $oldscoid;
                            $id = update_record('rscorm_scoes',$sco);
                            delete_records('rscorm_scoes_data','scoid',$oldscoid);
                        ...
                        if (!empty($id)) {
                            ...
                            if (isset($element->web_launch)) {
                                ...
                                $dataid = insert_record('rscorm_scoes_data',$scodata);
                            }
    ...
    if (!empty($oldscoes)) {
        foreach($oldscoes as $oldsco) {
            delete_records('rscorm_scoes','id',$oldsco->id);
            delete_records('rscorm_scoes_track','scoid',$oldsco->id);
        }
    }
...
function rscorm_get_toc($user,$scorm,$liststyle,$currentorg='',$scoid='',$mode='normal',$attempt='',$play=false) {
    global $CFG;

    $strexpand = get_string('expcoll','rscorm');
    $modestr = '';
    if ($mode == 'browse') {
        $modestr = '&amp;mode='.$mode;
    }
    ...
    //
    if (!empty($currentorg)) {
        if (($organizationtitle = get_field('rscorm_scoes','title','scorm',$scorm->id,'identifier',$currentorg)) != '') {
                ...
                if ($isvisible) {
                    ...
                    if (isset($usertracks[$sco->identifier])) {
                        ...
                        if ($sco->scormtype == 'sco') {
                            ...
                            $statusicon = '<img src="'.$scormpixdir.'/assetc.gif" alt="'.get_string('assetlaunched','rscorm').'" title="'.get_string('assetlaunched','rscorm').'" />';
                        }
                        ...
                        $incomplete = true;
                        if ($sco->scormtype == 'sco') {
                            $statusicon = '<img src="'.$scormpixdir.'/notattempted.gif" alt="'.get_string('notattempted','rscorm').'" title="'.get_string('notattempted','rscorm').'" />';
                        } else {
                            $statusicon = '<img src="'.$scormpixdir.'/asset.gif" alt="'.get_string('asset','rscorm').'" title="'.get_string('asset','rscorm').'" />';
                        }
                    }
                    if ($sco->id == $scoid) {
                        $startbold = '<b>';
                    ...
                    if (empty($sco->prerequisites) || rscorm_eval_prerequisites($sco->prerequisites,$usertracks)) {
                        if ($sco->id == $scoid) {
                            $result->prerequisites = true;
                        }
    ...
    $url = $CFG->wwwroot.'/mod/rscorm/player.php?a='.$scorm->id.'&amp;currentorg='.$currentorg.$modestr.'&amp;scoid=';
