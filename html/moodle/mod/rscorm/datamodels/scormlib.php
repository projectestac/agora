...
function rscorm_parse_scorm($pkgdir,$scormid) {
    global $CFG, $USER;
...
//print_object($scoes);
        if (count($scoes->elements) > 0) {
            $olditems = get_records('rscorm_scoes','scorm',$scormid);
...
// MARSUPIAL ********** MODIFICAT - Compare old item with new, and if something change update else do nothing
                    	...
                    	$update=false;
                    	if (!$olditemid || ($olditemid && $olditem->scorm!=$scormid)){
...
// ********* FI
                    	...
                    	if(!$olditemid){
                            ...
                            $id = insert_record('rscorm_scoes',$newitem);
                            if (!empty($olditems) && $olditemid) {
                                $olditems[$olditemid]->newid = $id;
                            }
                    	}else if($update){
                    	    $newitem->id=$id=$olditem->id;
                    	    update_record('rscorm_scoes',$newitem);
                    	}
                    	else{
                    	   	$id=$olditemid;
                    	}	
                    	if (isset($item->launch)&&$item->launch!=''){
                    		...
                    		if (!$scouserid=get_record('rscorm_scoes_users','scormid',$scormid,'scoid',$id,'userid',$USER->id)){
                    		    $scouser->scormid=$scormid;
                    		    $scouser->scoid=$id;
                    		    $scouser->userid=$USER->id;
                    	        $scouser->launch=$item->launch;
                    	        $scouser->timecreated=time();
                    	        $scouserid=insert_record('rscorm_scoes_users',$scouser);
                    		}else{
                    			$scouser->id=$scouserid->id;
                    			$scouser->launch=$item->launch;
                    			$scouser->timemodified=time();
                    			update_record('rscorm_scoes_users',$scouser);
                    		}
                    	}
...
//********** FI         
                        if ($optionaldatas = rscorm_optionals_data($item,$standarddatas)) {
                            ...
                            foreach ($optionaldatas as $optionaldata) {
                                if (isset($item->$optionaldata)) {
                                    ...
                                    $dataid = insert_record('rscorm_scoes_data',$data);
                                }
                            }
                        }
                        ...
                        if (isset($item->sequencingrules)) {
                            foreach($item->sequencingrules as $sequencingrule) {
                                ...
                                $ruleid = insert_record('rscorm_seq_ruleconds',$rule);
                                if (isset($sequencingrule->ruleconditions)) {
                                    foreach($sequencingrule->ruleconditions as $rulecondition) {
                                        ...
                                        $rulecondid = insert_record('rscorm_seq_rulecond',$rulecond);
                                    }
                                }
                            }
                        }
                        ...
                        if (isset($item->rolluprules)) {
                            foreach($item->rolluprules as $rolluprule) {
                                ...
                                $rollupruleid = insert_record('rscorm_seq_rolluprule',$rollup);
                                if (isset($rollup->conditions)) {
                                    foreach($rollup->conditions as $condition){
                                        ...
                                        $conditionid = insert_record('rscorm_seq_rolluprulecond',$cond);
                                    }
                                }
                            }
                        }
                        ...
                        if (isset($item->objectives)) {
                            foreach($item->objectives as $objective) {
                                ...
                                $objectiveid = insert_record('rscorm_seq_objective',$obj);
...
//print_object($objective->mapinfos);
                                    foreach($objective->mapinfos as $objmapinfo) {
                                        ...
                                        $mapinfoid = insert_record('rscorm_seq_mapinfo',$mapinfo);
                                    }
                                }
                            }
                        }
...
// MARSUPIAL ********** ELIMINAT - Taked out becouse is better to update than delete all and add it again each time
            /*if (!empty($olditems)) {
                foreach($olditems as $olditem) {
                    delete_records('rscorm_scoes','id',$olditem->id);
                    delete_records('rscorm_scoes_data','scoid',$olditem->id);
                    if (isset($olditem->newid)) {
                        set_field('rscorm_scoes_track', 'scoid', $olditem->newid, 'scoid', $olditem->id);
                    }
                    delete_records('rscorm_scoes_track','scoid',$olditem->id);
                    delete_records('rscorm_seq_objective','scoid',$olditem->id);
                    delete_records('rscorm_seq_mapinfo','scoid',$olditem->id);
                    delete_records('rscorm_seq_ruleconds','scoid',$olditem->id);
                    delete_records('rscorm_seq_rulecond','scoid',$olditem->id);
                    delete_records('rscorm_seq_rolluprule','scoid',$olditem->id);
                    delete_records('rscorm_seq_rolluprulecond','scoid',$olditem->id);
                }
            }*/
//********** FI
            if (empty($scoes->version)) {
...
function rscorm_is_leaf($sco) {
    if (get_record('rscorm_scoes','scorm',$sco->scorm,'parent',$sco->identifier)) {
        return false;
    }
    return true;
}
...
function rscorm_get_parent($sco) {
    if ($sco->parent != '/') {
        if ($parent = get_record('rscorm_scoes','scorm',$sco->scorm,'identifier',$sco->parent)) {
...
function rscorm_get_children($sco) {
    if ($children = get_records('rscorm_scoes','scorm',$sco->scorm,'parent',$sco->identifier)) {//originally this said parent instead of childrean
        return $children;
    }
    return null;
}

function rscorm_get_available_children($sco){
    $res = get_record('rscorm_scoes_track','scoid',$scoid,'userid',$userid,'element','availablechildren');
    if(!$res || $res == null){
        return false;
    }
    else{
...
function rscorm_get_siblings($sco) {
    if ($siblings = get_records('rscorm_scoes','scorm',$sco->scorm,'parent',$sco->parent)) {
...
function rscorm_get_ancestors($sco) {
    if ($sco->parent != '/') {
        return array_push(rscorm_get_ancestors(rscorm_get_parent($sco)));
    } else {
        return $sco;
    }
}
