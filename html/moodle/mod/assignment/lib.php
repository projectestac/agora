    ...
    */
    ...
    /// construct SQL, using current offset to find the data of the next student
        ...
        if ($users) {
            ...
            if ($sort = flexible_table::get_sql_sort('mod-assignment-submissions')) {
				//XTEC ********** AFEGIT -> Hacked to avoid "ORA-00932: inconsistent datatypes: expected - got CLOB" error
				//2011.10.17 @fcasanel - https://projectes.lafarga.cat/projects/agora/tracker/633/2089/detail
				$sort = str_replace('submissioncomment', 'TO_CHAR(submissioncomment)', $sort);
				//************** FI
                $sort = 'ORDER BY '.$sort.' ';
            }
    ...
    /// Construct the SQL
        ...
        if ($sort = $table->get_sql_sort()) {
            //XTEC ********** AFEGIT -> Hacked to avoid "ORA-00932: inconsistent datatypes: expected - got CLOB" error
            //2011.10.14 @sarjona - https://projectes.lafarga.cat/projects/agora/tracker/633/2089/detail
            $sort = str_replace('submissioncomment', 'TO_CHAR(submissioncomment)', $sort);
            //************** FI
            $sort = ' ORDER BY '.$sort;
        }
