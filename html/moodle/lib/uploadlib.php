...
class upload_manager {
    ...
    */
    ...
    function validate_file(&$file) {
        ...
        //XTEC ************** AFEGIT - Check if disk quote is exceeded
        //2011.06.15 @fcasanel
        global $CFG;
        if($CFG->diskPercent){
            if ($CFG->diskPercent >= 100){
		$file['uploadlog'] .= "\n".get_string('diskquoteerror', 'moodle');
		return false;
            }			
	}
        //************ FI
...
function upload_print_form_fragment($numfiles=1, $names=null, $descriptions=null, $uselabels=false, $labelnames=null, $coursebytes=0, $modbytes=0, $return=false) {
    ...
    for ($i = 0; $i < $numfiles; $i++) {
        ...
        //XTEC **************** MODIFICAT - To show alert message if there isn't free disk space
		//2010.07.15 @fcasanel
        if ($CFG->diskPercent > 100){
			$str .= '<input type="file" size="50" name="'. $name .'" alt="'. $name .'" DISABLED /><br />'."\n".'<span style="font-size:small; text-align:left !important;">'.get_string('diskquoteerror', 'moodle').'</span><br />';
		}else{
