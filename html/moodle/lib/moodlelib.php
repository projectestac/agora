...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml='', $attachment='', $attachname='', $usetrueaddress=true, $replyto='', $replytoname='', $wordwrapwidth=79) {
    ...
    //XTEC ******************* AFEGIT - Check email format
    //2011.06.21 @fcasanel
    $match = preg_match('/^([a-zA-Z0-9_\\-\\.]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([a-zA-Z0-9\\-]+\\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$/', $user->email);
    if(!$match){
		return false;
	}
    //******************* FI
...
/// We are going to use textlib services here
	...
	//XTEC ************ MODIFICAT - To identify the messages with the school name
	//2011.04.21 @fcasanellas
        if (!isset($CFG->mailheader)) {
            $CFG->mailheader = '';
        }
        if($CFG->apligestmail){
            $mail->Subject = $CFG->mailheader." [".get_site()->fullname."] ".substr(stripslashes($subject), 0, 900);
        }else{
            $mail->Subject = $CFG->mailheader." ".substr(stripslashes($subject), 0, 900);
        }
	//************ ORIGINAL
