...
/// Get all required strings
    ...
    $strconsult = get_string('logins', 'block_admin_service');
	...
	$form.="<TR>";
	$form.='<TD colspan="3">'.get_string('logats', 'block_admin_service').'</TD></TR>';
	...
	/** 
	 * Cambio para restringir las fechas de consulta, para que el Enric no haga KILLER QUERYS
	 * Añadido
	 **/
	...
	/**
	 *Cambio por defecto, pero quitado. Se cambio de Idea. 
	 $form.='<INPUT type ="hidden" name= "indform[tot]"">' .
			'<INPUT type ="hidden" name= "indform[dia_fin]" value="'.date("j").'">' .
			'<INPUT type ="hidden" name= "indform[mes_fin]" value="'.date("n").'">' .
			'<INPUT type ="hidden" name= "indform[any_fin]" value="'.date("Y").'">' .
			'<INPUT type ="submit" name= "indform[button]" value="'.get_string('search').'"><br />
			</FORM>';*/
	$form.='</TD></TR>';		
	$form.='</TABLE>';
		...
		$results = array();
			if (isset ($indform['tot'])){
				...
				if(generic_connect()){
					...
					$query="SELECT * FROM {$prefix}log WHERE time BETWEEN $start_time AND $now AND action = 'login' ORDER BY time ASC";
