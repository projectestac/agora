...
/// Get all required strings
    ...
    $strconsult = get_string('loginsuserko', 'block_admin_service');
	...
	$form.="<TR>";
	$form.='<TD colspan="3">'.get_string('loginsuserko', 'block_admin_service').'</TD></TR>';
	...
	if (isset ($indform['button'])){
		...
		if (!isset ($ADM->entorn_actual)){
			...
			if (isset ($indform['tot'])){
				...
				//$query= "SELECT * FROM {$CFG->prefix}log WHERE time BETWEEN $start_time AND $now AND module = 'login' AND action = 'error' ORDER BY time ASC";
				//echo $query;
				//$results = get_records_sql($query);
				
				if(generic_connect()){
					$prefix = generic_prefix();
					$query="SELECT * FROM {$prefix}log WHERE time BETWEEN $start_time AND $now AND module = 'login' AND action = 'error' ORDER BY time ASC";
