...
switch ($action){
	case 'manage':
		...
		$book = get_record('rcommon_books', 'id', $id);
		$credentials = get_records_sql("SELECT ruc.id, ruc.credentials, ruc.euserid, u.lastname, u.firstname FROM {$CFG->prefix}rcommon_user_credentials ruc LEFT JOIN {$CFG->prefix}user u ON ruc.euserid = u.id WHERE isbn = '{$book->isbn}' ORDER BY u.lastname, u.firstname");
		...
		<table cellpading="5" width="100%">
			<tr>
				<th>&nbsp;</th>
				<th>' . get_string('credential', 'block_rcommon') . '</th>
				<th>' . get_string('user', 'block_rcommon') . '</th>
				<th>' . get_string('actions', 'block_rcommon') . '</th>
				<th>' . get_string('comments', 'block_rcommon') . '</th>
			</tr>';
		if (empty($credentials)){
			...
			foreach ($credentials as $credential){
				...
				echo '<tr>
					...
					echo '</td>
					<td><img id="loading_small_' . $credential->id . '" src="' . $CFG->themewww . '/' .$CFG->theme . '/pix/i/loading_small.gif" width="16" height="16" alt="Loading small image" style="visibility: hidden; float: left;"><span id="desc_' . $credential->id . '"></span</td>
				</tr>';
			}
		}
		echo '</table>';
	...
	case 'edit':
		...
		$book       = get_record('rcommon_books', 'id', $id_bk);
	...
	case 'doEdit':
		...
		echo '<p class="center">(' . get_string('keymanager_doEdit_message', 'block_rcommon') . ' <a href="keyManager_new.php?action=manage&id_bk=' . $id_book . '" title="' . get_string('goback_title', 'block_rcommon') . '">' . get_string('click_hear', 'block_rcommon') . '<a>)</p>
	...
	case 'unassing':
		...
		$ids_where = implode(',', $ids);
	   	if (execute_sql("UPDATE {$CFG->prefix}rcommon_user_credentials SET euserid = '0', timemodified = '" . time() . "' WHERE id IN ({$ids_where})", false)){
	   	...
	   	echo '<p class="center">(' . get_string('keymanager_doEdit_message', 'block_rcommon') . ' <a href="keyManager_new.php?action=manage&id_bk=' . $id_bk . '" title="' . get_string('goback_title', 'block_rcommon') . '">' . get_string('click_hear', 'block_rcommon') . '<a>)</p>
	...
	case 'assing':
		...
		$ids_cnt = is_array($ids)? count($ids): count(explode(',', $ids));
		...
		if ($frm = data_submitted()){
			if (!empty($frm->addselect) && confirm_sesskey()) {			
				...
				$empty_credentials = get_records_sql("SELECT id, isbn, credentials FROM {$CFG->prefix}rcommon_user_credentials WHERE isbn = (SELECT isbn FROM {$CFG->prefix}rcommon_books WHERE id = '{$id_bk}') AND euserid = '0' AND id IN ({$frm->ids})");
				...
				if ($empty_credentials){
					...
					foreach($empty_credentials as $c){
						...
						if ($user_c = get_record_sql("SELECT id, credentials FROM {$CFG->prefix}rcommon_user_credentials WHERE isbn = '{$c->isbn}' AND euserid = '{$frm->addselect[$i]}'", true)){
						...
						$up->timemodified = time();
						if (update_record('rcommon_user_credentials', $up)){
							$i++;
						}
					}
				}
				...
				foreach ($frm->removeselect as $u){
					$cred = get_record_sql("SELECT id FROM {$CFG->prefix}rcommon_user_credentials WHERE euserid = '{$u}' AND isbn = (SELECT isbn FROM {$CFG->prefix}rcommon_books WHERE id = '{$id_bk}')");
					if ($cred){
						$up          = new stdClass();
						$up->id      = $cred->id;
						$up->euserid = 0;
		...
		$already_asigned_users     = get_records_sql("SELECT c.euserid as id, u.firstname, u.lastname, u.email FROM {$CFG->prefix}user u RIGHT JOIN {$CFG->prefix}rcommon_user_credentials c ON u.id = c.euserid WHERE c.id IN ({$ids}) AND euserid NOT LIKE '0'");
...
// ************ FI
				...
				<td valign="top">
		      	...
		      	<td valign="top">';	          	
		        	...
		        	echo '<p class="arrow_button">
		            	<input name="add" id="add" type="submit" value="' . $THEME->larrow.'&nbsp;'.get_string('keymanager_assignaction', 'block_rcommon') . '" title="'. get_string('add') . '"';
		            	if ($already_unassigned == 0){
		            		echo ' disabled="true"';
		            	}
		            	echo '/><br />
		            	<input name="remove" id="remove" type="submit" value="' . get_string('keymanager_unassignaction', 'block_rcommon').'&nbsp;'.$THEME->rarrow . '" title="' . get_string('remove') . '" /><br><br>
		            	<input name="add" id="addall" type="submit" onclick="select_all();" value="' . $THEME->larrow.'&nbsp;'.get_string('keymanager_assignaction_all', 'block_rcommon') . '" title="'. get_string('add') . '"';
		            	if ($already_unassigned == 0){
		            		echo ' disabled="true"';
		            	}
		            	echo '/><br /><input name="remove" id="removeall" type="submit" onclick="unselect_all();" value="' . get_string('keymanager_unassignaction_all', 'block_rcommon').'&nbsp;'.$THEME->rarrow . '" title="' . get_string('remove') . '" /><br />
		        	</p>
		      	</td>
		      	<td valign="top">
	...
	case 'doAdd':
		...
		if (empty($confirm)){
			$bk = get_record('rcommon_books', 'id', $id_bk);
			$cred = get_record_sql("SELECT id FROM {$CFG->prefix}rcommon_user_credentials WHERE isbn = '{$bk->isbn}' AND credentials = '{$new_credential}'", true);
			if ($cred){
		...
		if (insert_record('rcommon_user_credentials', $insert)){
	...
	case 'import':
		...
		echo '<p>' . get_string('keymanager_import_explanation', 'block_rcommon', $post_max_size) . '                    
                <span class="helplink"><a title="Ajuda" href="#" onclick="this.target=\'popup\'; return openpopup(\'/blocks/rcommon/lang/'.$CFG->lang.'/help/rcommon/importcsv.html\',  \'popup\', \'menubar=0,location=0,scrollbars,resizable,width=500,height=400\', 0);"><img class="iconhelp" alt="Ajuda" src="'.$CFG->wwwroot.'/pix/help.gif"></a></span>
                </p>
		...
		if ($file) {
			...
			while (!feof($file)) {
				...
				if ($i == 0){
					...
					if (empty($data[$columns['isbn']])){
						...
						if ($bk = get_record('rcommon_books', 'isbn', $data[$columns['isbn']])){
					...
					if (isset($columns['username']) || isset($columns['userid'])){
						...
						if (empty($data[$columns['username']]) && empty($data[$columns['userid']])){
							...
							if ($user = get_record('user', $user_c, $user_v, '', '', '', '', 'id, username')){
					...
					$duplicated = false;
					if (isset($user) && $user){
						if ($cred = get_record_sql("SELECT id FROM {$CFG->prefix}rcommon_user_credentials WHERE isbn = '{$data[$columns['isbn']]}' AND euserid = '{$user->id}'", true)){
					...
					if (!$duplicated){
						if (empty($data[$columns['credential']])){
							...
							if (isset($user) && $user){
								...
								if ($cred = get_record_sql("SELECT id FROM {$CFG->prefix}rcommon_user_credentials WHERE isbn = '{$data[$columns['isbn']]}' AND credentials = '{$data[$columns['credential']]}' AND euserid = '{$user->id}'", true)){
									...
									if ($creds = get_records_sql("SELECT id FROM {$CFG->prefix}rcommon_user_credentials WHERE isbn = '{$data[$columns['isbn']]}' AND credentials = '{$data[$columns['credential']]}'")){
										$a->cnt = count($creds);
										$a->bk  = $data[$columns['isbn']];
			...
			if ($critical_error){
				...
				if (count($rows_error) != 0){
					...
					echo '<br><p class="center"><input type="submit" value="' . get_string('continue') . '"> <a href="keyManager_new.php?action=import"><button>' . get_string('back') . '</button></a></p></form>';
				} else {
					echo '<form id="frm" name="frm" action="keyManager_new.php?action=importstep3" method="post" class="center">
					<input type="hidden" name="columns" value=\'' . serialize($columns) . '\'>
					<input type="hidden" name="rows" value=\'' . serialize($rows) . '\'>
	...
	case 'importstep3':
		...
		<form action="keyManager_new.php?action=importstep4" method="post">
			...
			<p class="center"><input type="submit" value="' . get_string('keymanager_import_button', 'block_rcommon') . '"> <a href="' . $CFG->wwwroot . '/blocks/rcommon/keyManager_new.php"><button>' . get_string('cancel') . '</button></a></p>
		</form>';
	...
	case 'importstep4':	
		...
		foreach($rows as $k => $v){
			...
			if (!insert_record('rcommon_user_credentials', $in)){
				$errors_cnt++;
				continue;
			}
		}
		...
		echo '<p class="center">' . get_string($msg, 'block_rcommon', $a) . ' <a href="keyManager_new.php" title="' . get_string('goback_title', 'block_rcommon') . '">' . get_string('click_hear', 'block_rcommon') . '<a>)</p>
...
LEFT JOIN {$CFG->prefix}rcommon_books rb ON data.isbn = rb.isbn LEFT JOIN {$CFG->prefix}rcommon_publisher rp ON rb.publisherid = rp.id ORDER BY ed_name, bk_name";
		...
		echo '<table cellpadding="5" width="100%">
			<tr>
				<th>' . get_string('publisher', 'block_rcommon') . '</th>
				<th>' . get_string('book', 'block_rcommon') . '</th>
				<th width="95">' . get_string('assigned', 'block_rcommon') . '</th>
				<th width="95">' . get_string('totals', 'block_rcommon') . '</th>
				<th width="95">' . get_string('actions', 'block_rcommon') . '</th>
			</tr>';
		if ($list_credentials){
