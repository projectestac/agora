...
if(!$site = get_site()) {
	error(get_string('accessdenied'), $CFG->wwwroot.'/'.$CFG->admin.'/index.php');
}
...
switch($action){
	case 'edit':
		...
		if (!$record = get_record('rcommon_publisher', 'id', $publisher)){
			error(get_string('nopublisher', 'block_rcommon'), $CFG->wwwroot.'/blocks/rcommon/publishersManager.php');
		}
	case 'add';
...
// *********** END
		...
		if (empty($record->name)){
			error(get_string('savekoemptyvalues', 'block_rcommon'), $CFG->wwwroot.'/blocks/rcommon/publishersManager.php');
		}
		...
		if (empty($publisher)){			
			...
			if (!insert_record('rcommon_publisher', $record)){
			...
			if (!update_record('rcommon_publisher', $record)){
			    redirect($CFG->wwwroot.'/blocks/rcommon/publishersManager.php?action=edit&publisher='.$publisher, get_string('saveko', 'block_rcommon'), 5);
			}		
		}
	...
	case 'del':
		...
		if (empty($confirm)){
			...
			if (!delete_records('rcommon_publisher', 'id', $publisher)){
	...
	break;
	default:
        	$publishers = get_records('rcommon_publisher', '', '', 'name ASC');
		if (!empty($publishers)){    
