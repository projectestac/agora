...
function get_all_books_structure($publisherid = 0, $isbn = '')
{
    ...
    try
    {
        ...
        if ($publishers = get_records_sql($select))
        {
    ...
    catch(Exception $fault) 
    {
        ...
        $tmp->info      =  'Error get_all_books_structure: '.$fault->faultstring;
...
//********** MODIFICAT MARSUPIAL - if there is no level saves
	                            ...
	                            //$level = get_record_sql("Select id FROM ".$CFG->prefix."rcommon_level WHERE code = '".$li["nivel"]["value"]."'");
...
//********** 
	                            ...
	                            if ($level = get_record_sql("Select id FROM ".$CFG->prefix."rcommon_level WHERE code = '".$nivel_libro."'"))
	                            {
	                                ...
	                                $levelid = insert_record('rcommon_level', $instance_level);
	...
	//**********    
	                            ...
	                            if (!$regbook = get_record_sql('SELECT id FROM '.$CFG->prefix.'rcommon_books' .' '. $select, true))
	                            {
	                                $instance->timecreated = time();
	                                $bookid = insert_record('rcommon_books', $instance);
	                                echo "OK";
	                            }
	                            else
	                            {
	                                ...
	                                $bookid = $regbook->id;
	                                update_record('rcommon_books', $instance);
	                                echo "OK";
	                            }
...
//********** FI
	                                    ...
	                                    if (!$regunit = get_record_sql('SELECT id FROM '.$CFG->prefix.'rcommon_books_units' .' '. $select, true))
	                                    {
	                                    	//echo "<br>sql: SELECT id FROM {$CFG->prefix}rcommon_books_units ".$select." -> INSERT";
	                                        $instance->timecreated = time();
	                                        $unitid = insert_record('rcommon_books_units', $instance);
	                                    }
	                                    else
	                                    {
	                                        ...
	                                        $unitid = $regunit->id;
	                                        update_record('rcommon_books_units', $instance);
	                                    }
...
//********* FI
	                                            ...
	                                            if (!$regactiv = get_record_sql('SELECT id FROM '.$CFG->prefix.'rcommon_books_activities' .' '. $select, true))
	                                            {
	                                                $instance->timecreated = time();
	                                                //echo "<br><br>"; print_r($instance); echo "<br><br>";
	                                                $activid = insert_record('rcommon_books_activities', $instance);
	                                            }
	                                            else
	                                            {
	                                                $instance->id = $regactiv->id;
	                                                $activid = $regactiv->id;
	                                                update_record('rcommon_books_activities', $instance);
	                                            }
	                                        }
	                                    }
	                                }
...
// ************** FI
	                            ...
	                            continue;
	                            //error('Error get_book_structure: C&oacute;digo: '.$ret->ObtenerEstructuraResult->Codigo.' - '.$ret->ObtenerEstructuraResult->Descripcion);
	                        }
	                    }
	                }
	                ...
	                return true;
	              	 ...
	              	 echo "<br><br>".get_string('nobooks','blocks/rcommon');
	              	 return true;
	              }
            }
            else
...
// ************** FI
                ...
                return false;
                //error('Error get_books: C&oacute;digo: '.$ret->ObtenerTodosResult->Codigo.' - '.$ret->ObtenerTodosResult->Descripcion);
            }
        }
    }
    catch(Exception $fault) 
...
// ************* FI
            ...
            {
                return $response;
                //error('Error get_books: C&oacute;digo: '.$response->ObtenerTodosResult->Codigo.' - '.$response->ObtenerTodosResult->Descripcion);
            }
            else
            {
                //return $response;
            ...
            error(get_string("centernotfound", "block_rcommon"));
    ...
    //catch(SoapFault $fault) 
    {
        ...
        //error('Error get_books: '.$fault->faultstring);
        
        echo "<br>Error get_books: ". $fault->getMessage() . " -- ". $fault->faultstring;
...
// ************* FI
        ...
        {
            //error('Error get_book_structure: C&oacute;digo: '.$response->ObtenerEstructuraResult->Codigo.' - '.$response->ObtenerEstructuraResult->Descripcion);
            return $response;
        }
        else
        {
        ...
        //echo "Error: ". $fault->faultstring;
        log_to_file("wsBookStructure: get_book_structure - Exception = ".$fault->faultstring,'rcommon_tracer');
        echo "KO!";
        ...
        //error('Error get_book_structure: '.$fault->faultstring);
    }    
