<?PHP // $Id: report_security.php,v 1.24 2010/07/20 19:44:51 mits Exp $ 
      // report_security.php - created with Moodle 2.0 dev (Build: 20100220) (2010021900)


$string['check_configrw_details'] = '<p>ウェブサーバからファイルが修正されないよう、インストール後、config.phpのファイルパーミッションを変更することをお勧めします。この方法はサーバのセキュリティを著しく向上させるものではありませんが、一般的なセキュリティ上の弱点 (exploits) をスローダウンしたり、制限することはできます。</p>';
...
$string['check_courserole_details'] = '<p>それぞれのコースではデフォルトの登録ロールが設定されています。このロールにリスクのあるケイパビリティが許可されているかどうか確認してください。</p>
...
$string['check_courserole_ok'] = 'コースデフォルトロール定義はOKです。';
...
$string['check_defaultcourserole_details'] = '<p>コース登録のデフォルト学生ロールはコースのデフォルトロールに指定されます。このロールにリスクのあるケイパビリティが許可されているかどうか確認してください。</p>
...
$string['check_defaultcourserole_ok'] = 'サイトデフォルトロールの定義はOKです。';
...
$string['check_displayerrors_details'] = '<p>いくつかのエラーメッセージはあなたのサーバに関する機密情報を漏洩する可能性があるため、PHP設定 <code>display_errors</code>の有効化は実稼動サイトではお勧めできません。</p>';
...
$string['check_emailchangeconfirmation_ok'] = 'メールの変更は確認されるべきです。';
...
$string['check_frontpagerole_ok'] = 'フロントページロールはOKです。';
$string['check_globals_details'] = '<p>Register globalsは極めて危険なPHP設定だと考えられています。</p>
<p>PHP設定において、<code>register_globals=off</code>が設定されるべきです。この設定はあなたの<code>php.ini</code>、Apache/IIS設定または<code>.htaccess</code>ファイルを編集することでコントロールされます。</p>';
$string['check_globals_error'] = 'Register globalsは無効にされるべきです。すぐにPHP設定を修正してください!';
$string['check_globals_name'] = 'Register globals';
$string['check_globals_ok'] = 'Register globalsが無効にされています。';
$string['check_google_details'] = '<p>Googleに公開する設定ではサーチエンジンがコースにゲストとしてログインできるようにします。ログインアクセスが許可されていない場合、この設定の有効化は意味を持ちません。</p>';
...
$string['check_noauth_details'] = '<p>「認証なし」プラグインは実稼動サイト向けではありません。このサイトが開発テストサイトではない限り、このプラグインを無効にしてください。</p>';
$string['check_noauth_error'] = '「認証なし」プラグインは実稼動サイトでは使用できません。';
...
<p>ランダム文字列は数字および他の文字を混ぜた形で記述してください。ストリング長は少なくとも40文字を推奨します。</p>
...
$string['check_passwordsaltmain_ok'] = 'パスワードSALTはOKです。';
$string['check_passwordsaltmain_warning'] = 'パスワードSALTは設定されていません。';
...
<p>システムコンテクストのみへの管理者ロール割り当てをお勧めします。以下のユーザはサポートされない管理者ロールが割り当てられています:</p>$a->unsupported';
...
$string['check_riskbackup_details_overriddenroles'] = '<p>これらのアクティブなオーバーライドはユーザに対して、バックアップ内にユーザデータを含むことができる能力を与えます。このパーミッションが本当に必要かどうか確認してください。</p> $a';
$string['check_riskbackup_details_systemroles'] = '<p>現在、以下のシステムロールはユーザに対して、バックアップ内にユーザデータを含むことを許可しています。このパーミッションが本当に必要かどうか確認してください。</p> $a';
...
$string['check_riskxss_details'] = '<p>RISK_XSSは信頼されるユーザのみ使用できる、すべての危険なケイパビリティを意味します。</p>
...
$string['check_unsecuredataroot_error'] = 'あなたのdatarootディレクトリ <code>$a</code> は正しくない場所に配置され、ウェブにさらされた状態です!';
$string['check_unsecuredataroot_name'] = '安全ではないdataroot';
$string['check_unsecuredataroot_ok'] = 'datarootディレクトリはウェブからアクセスできないようにしてください。';
$string['check_unsecuredataroot_warning'] = 'あなたのdatarootディレクトリ <code>$a</code> は正しくない場所に配置され、ウェブにさらされた状態です。';
