...
class Formicula_Controller_User extends Zikula_AbstractController {
    ...
    public function main($args=array()) {
        ...
        if ($cid == -1) {
            $contacts = ModUtil::apiFunc('Formicula', 'user', 'readValidContacts', array('form' => $form));
        } else {
            $contacts[] = ModUtil::apiFunc('Formicula', 'user', 'getContact', array('cid' => $cid,
                        'form' => $form));
        }
    ...
    public function send($args=array()) {
        ...
        // Upload directory
        // * XTEC AFEGIT - Posem el directori temporal del servidor
        // Albert 7/06/2013
        $uploaddir = System::getVar('temp');
        if (StringUtil::right($uploaddir, 1) <> '/') {
            $uploaddir .= '/';
        }
        $uploaddir .='formicula_cache';
        // FI AFEGIT
        ...
        $contact = ModUtil::apiFunc('Formicula', 'user', 'getContact', array('cid' => $cid,
                    'form' => $form));
        ...
        if (ModUtil::apiFunc('Formicula', 'user', 'checkArguments', array('userdata' => $userdata,
                    'custom' => $custom,
                    'userformat' => $userformat)) == true) {
            // send the submitted data to the contact(s)
            if (ModUtil::apiFunc('Formicula', 'user', 'sendtoContact', array('contact' => $contact,
                        'userdata' => $userdata,
                        'custom' => $custom,
                        'form' => $form,
            ...
            if (($this->getVar('send_user') == 1) && ($userformat <> 'none')) {
                ...
                $this->view->assign('sendtouser', ModUtil::apiFunc('Formicula', 'user', 'sendtoUser', array('contact' => $contact,
                            'userdata' => $userdata,
                            'custom' => $custom,
                            'form' => $form,
                            'format' => $userformat)));
            ...
            if ($store_data == 1) {
                ...
                if (empty($store_data_forms) || (is_array($store_data_forms_arr) && in_array($form, $store_data_forms_arr))) {
                    ModUtil::apiFunc('Formicula', 'user', 'storeInDatabase', array('contact' => $contact,
                        'userdata' => $userdata,
                        'custom' => $custom,
                        'form' => $form));
                }
