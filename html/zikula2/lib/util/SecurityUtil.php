...
class SecurityUtil
{
    ...
    public static function checkSaltedHash($unhashedData, $saltedHash, array $hashMethodCodeToName = array(), $saltDelimeter = self::SALT_DELIM)
    {
        ...
        if (is_string($unhashedData) && is_string($saltedHash) && is_string($saltDelimeter) && (strlen($saltDelimeter) == 1)
            ...
            // 2013.09.17 @aginard
            // 2014.09.29 @aginard: Modified to support passwords from Moodle 2.6+

            global $ZConfig;

            if (strlen($saltedHash) == 60) {
                if (!defined('PASSWORD_BCRYPT')) {
                    define('PASSWORD_BCRYPT', 1);
                }
                $dataMatches = SecurityUtil::password_verify($unhashedData, $saltedHash);
                var_dump($dataMatches);
            } 
            elseif (isset($ZConfig['MoodleSalt'])) {
    ...
    /**
     * Copied function hash from Moodle 2.6+
     * 
     * @param type $password
     * @param type $algo
     * @param array $options
    ...
    /**
     * Copied function to verify hash from Moodle 2.6+
     * 
     * @param type $password
     * @param type $hash
     * @return boolean
