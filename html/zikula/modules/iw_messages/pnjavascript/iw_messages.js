...
function mark(msg_id){
	var pars = "module=iw_messages&func=mark&msg_id=" + msg_id;
...
function marcar(){
	...
	var pars = "module=iw_messages&func=marca&ids=" + param + "&num=" + $("total_messages").value;
	var myAjax = new Ajax.Request("ajax.php", 
	{
		method: 'get', 
		parameters: pars, 
...
function esborrar(text){
	...
	if(resposta){
		...
		var pars = "module=iw_messages&func=delete&ids=" + param + "&num=" + $("total_messages").value+"&qui=d";
		var myAjax = new Ajax.Request("ajax.php", 
		{
			method: 'get', 
			parameters: pars, 
...
function esborrarsend(text){
	...
	if(resposta){
		...
		var pars = "module=iw_messages&func=delete&ids=" + param + "&num=" + $("total_messages_send").value+"&qui=r";
		var myAjax = new Ajax.Request("ajax.php", 
		{
			method: 'get', 
			parameters: pars, 
...
function esborrardisplay(text){
	resposta=confirm(text);
	if(resposta){document.prvmsg.submit();}		
}


function marcardisplay(id){

	var pars = "module=iw_messages&func=marca&ids=" + id + "&num=1";
	var myAjax = new Ajax.Request("ajax.php", 
	{
		method: 'get', 
		parameters: pars, 
...
function autocompleteUser(value){
	if(value.length > 1){
		var pars = "module=iw_messages&func=autocompleteUser&value=" + value;
		showAutoCompete();
		var myAjax = new Ajax.Request("ajax.php", 
		{
			method: 'get', 
...
function view(inici,rpp,inicisend,rppsend,filter,filtersend){
	var pars = "module=iw_messages&func=view&inici=" + inici + "&rpp=" + rpp + "&inicisend=" + inicisend + "&rppsend=" + rppsend + "&filter=" + filter + "&filtersend=" + filtersend;
	var myAjax = new Ajax.Request("ajax.php", 
	{
		method: 'get', 
		parameters: pars, 
...
function view_response(req){
	...
	var json = pndejsonize(req.responseText);
	Element.update('iw_mainContent', json.content).innerHTML;
}


function display(msgid, qui, inici, rpp, inicisend, rppsend, filter, filtersend){
	var pars = "module=iw_messages&func=display&msgid=" + msgid + "&qui=" + qui + "&inici=" + inici + "&rpp=" + rpp + "&inicisend=" + inicisend + "&rppsend=" + rppsend + "&filter=" + filter + "&filtersend=" + filtersend;
//	wait();
	var myAjax = new Ajax.Request("ajax.php", 
	{
		method: 'get', 
...
function displaysend(msgid, qui, uid, inici, rpp, inicisend, rppsend, filtersend, filter){
	var pars = "module=iw_messages&func=displaysend&msgid=" + msgid + "&qui=" + qui + "&uid=" + uid + "&inici=" + inici + "&rpp=" + rpp + "&inicisend=" + inicisend + "&rppsend=" + rppsend + "&filtersend=" + filtersend + "&filter=" + filter;
	var myAjax = new Ajax.Request("ajax.php", 
	{
		method: 'get', 
		parameters: pars, 
