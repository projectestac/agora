...
// Delete all the values that have not been used during the time specified in the config values
...
function userNews() {
	...
	//XTEC ************ MODIFICAT - Afegit nom del centre i canvi d'idioma de l'assumpte dels correus al català
	//2011.05.25 @Aida Regi
	$sitename = pnConfigGetVar('sitename');
	$subject = __('['.$sitename.']'.' Novetats publicades a la intranet del centre', $dom);
		...
		if($value != ''){
			...
			if($subscribed){
				...
				if($newsValue != __('Nothing to show', $dom)){
					...
					$newsValueText = '<div>' .pnModGetVar('iw_main', 'cronHeaderText').'</div> <br/> ';
					$newsValueText .= '<table width="300">'.$newsValue.'</table><br/>';
