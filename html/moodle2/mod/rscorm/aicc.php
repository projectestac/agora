...
$url = new moodle_url('/mod/rscorm/aicc.php', array('command'=>$command, 'session_id'=>$sessionid));
if ($aiccdata !== 0) {
...
if (empty($cfg_scorm->allowaicchacp)) {
    ...
    $scormsession = rscorm_aicc_confirm_hacp_session($sessionid);
    if (empty($scormsession)) {
        print_error('invalidhacpsession', 'rscorm');
    }
...
if (!empty($command)) {
    ...
    if ($sco = rscorm_get_sco($scoid, SCO_ONLY)) {
        if (!$scorm = $DB->get_record('rscorm', array('id'=>$sco->scorm))) {
            print_error('cannotcallscript');
        }
    } else {
        print_error('cannotcallscript');
    ...
    $aiccrequest = "MOODLE scoid: $scoid"
                 ...
                 . "\r\nAICC aiccdata:\r\n$aiccdata";
    rscorm_debug_log_write("aicc", "HACP Request:\r\n$aiccrequest", $scoid);
    ob_start();
        ...
        switch ($command) {
            case 'getparam':
                ...
                if ($status != 'Running') {
                    ...
                    if ($usertrack=rscorm_get_tracks($scoid, $aiccuser->id, $attempt)) {
                        $userdata = $usertrack;
                    } else {
                        $userdata->status = '';
                        $userdata->score_raw = '';
                    ...
                    if ($sco = rscorm_get_sco($scoid)) {
                        ...
                        if (isset($userdata->{'cmi.core.lesson_status'})) {
                            ...
                            $scormsession->rscorm_lessonstatus = $userdata->{'cmi.core.lesson_status'};
                        } else {
                            echo 'Lesson_Status=not attempted'.$userdata->entry."\r\n";
                            $scormsession->rscorm_lessonstatus = 'not attempted';
                        }
            ...
            case 'putparam':
                if ($status == 'Running') {
                    if (! $cm = get_coursemodule_from_instance("rscorm", $scorm->id, $scorm->course)) {
                        echo "error=1\r\nerror_text=Unknown\r\n"; // No one must see this error message if not hacked
                    }
                    if (!empty($aiccdata) && has_capability('mod/rscorm:savetrack', get_context_instance(CONTEXT_MODULE, $cm->id), $aiccuser->id)) {
                        $initlessonstatus = 'not attempted';
                        $lessonstatus = 'not attempted';
                        if (isset($scormsession->rscorm_lessonstatus)) {
                            $initlessonstatus = $scormsession->rscorm_lessonstatus;
                        }
                        $score = '';
                                ...
                                if (isset($datamodel[$element])) {
                                    ...
                                    switch ($element) {
                                        case 'cmi.core.lesson_location':
                                            $id = rscorm_insert_track($aiccuser->id, $scorm->id, $sco->id, $attempt, $element, $value);
                                        break;
                                        case 'cmi.core.lesson_status':
                                            ...
                                            if (empty($value) || isset($exites[$value])) {
                                                ...
                                                $id = rscorm_insert_track($aiccuser->id, $scorm->id, $sco->id, $attempt, $subelement, $value);
                                            }
                        ...
                        if (($mode == 'browse') && ($initlessonstatus == 'not attempted')) {
                            ...
                            $id = rscorm_insert_track($aiccuser->id, $scorm->id, $sco->id, $attempt, 'cmi.core.lesson_status', 'browsed');
                        }
                        if ($mode == 'normal') {
                            if ($sco = rscorm_get_sco($scoid)) {
                                ...
                                $id = rscorm_insert_track($aiccuser->id, $scorm->id, $sco->id, $attempt, 'cmi.core.lesson_status', $lessonstatus);
                            }
                        }
                    }
                    echo "error=0\r\nerror_text=Successful\r\n";
            ...
            case 'exitau':
                if ($status == 'Running') {
                    if (isset($scormsession->sessiontime) && ($scormsession->sessiontime != '')) {
                        if ($track = $DB->get_record('rscorm_scoes_track', array("userid"=>$aiccuser->id,
                                                                                "scormid"=>$scorm->id,
                                                                                "scoid"=>$sco->id,
                                                                                "attempt"=>$attempt,
                                                                                "element"=>'cmi.core.total_time'))) {
                            // Add session_time to total_time
                            $value = rscorm_add_time($track->value, $scormsession->sessiontime);
                            $track->value = $value;
                        ...
                        rscorm_update_grades($scorm, $aiccuser->id);
                    }
...
if (empty($cfg_scorm->allowaicchacp)) {
    $SESSION->rscorm = $scormsession;
} else {
    $scormsession->timemodified = time();
    $DB->update_record('rscorm_aicc_session', $scormsession);
}

$aiccresponse = ob_get_contents();
rsscorm_debug_log_write("aicc", "HACP Response:\r\n$aiccresponse", $scoid);
ob_end_flush();