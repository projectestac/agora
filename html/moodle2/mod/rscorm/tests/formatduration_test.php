...
/**
 * Unit tests for rscorm_formatduration function from locallib.php
 *
 * @package    mod_rscorm
 * @category   phpunit
...
global $CFG;
require_once($CFG->dirroot . '/mod/rscorm/locallib.php'); // Include the code to test


class mod_rscorm_formatduration_testcase extends basic_testcase {
    function test_rscorm2004_format() {
        ...
        foreach ($SUTs as $key => $SUT) {
            $formatted = rscorm_format_duration($SUT);
            $this->assertEquals($formatted, $validates[$key]);
        }
    }

    function test_rscorm12_format() {
        $stryears = get_string('years');
        $strmonths =  trim(get_string('nummonths'));
        $strdays = get_string('days');
        $strhours = get_string('hours');
