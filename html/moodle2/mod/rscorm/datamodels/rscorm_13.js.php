...
// Used need to debug cmi content (if you uncomment this, you must comment the definition inside RSCORMapi1_3)
//var cmi = new Object();

//
// RSCORM 1.3 API Implementation
//
function RSCORMapi1_3() {
...
<?php
    ...
    rscorm_reconstitute_array_element($scorm->version, $userdata, 'cmi.objectives', array('score'));
    rscorm_reconstitute_array_element($scorm->version, $userdata, 'cmi.interactions', array('objectives', 'correct_responses'));
    rscorm_reconstitute_array_element($scorm->version, $userdata, 'cmi.comments_from_learner', array());
    rscorm_reconstitute_array_element($scorm->version, $userdata, 'cmi.comments_from_lms', array());
?>
...
<?php
    ...
    include_once($CFG->dirroot.'/mod/rscorm/datamodels/callback.js.php');
 ?>
                ...
                if ('true' == result) {
                    ...
                    if (adl.nav.request != '_none_') {
                        switch (adl.nav.request) {
                            case 'continue':
                                setTimeout('rscorm_get_next();',500);
                            break;
                            case 'previous':
                                setTimeout('rscorm_get_prev();',500);
                            break;
                            case 'choice':
                            break;
                            case 'exit':
                    ...
                    if(<?php echo $CFG->branch; ?> < 24 )  {
                        ...
                        YUI().use('yui2-json', 'yui2-connection', 'yui2-event', function(Y) {
                            Y.YUI2.util.Connect.asyncRequest('GET', sURL, this.connectPrereqCallback, null)
                        });
                    }
...
//            }
    ...
    function StoreData(data,storetotaltime) {
        ...
        var result = DoRequest(myRequest,"<?php p($CFG->wwwroot) ?>/mod/rscorm/datamodel.php","id=<?php p($id) ?>&a=<?php p($a) ?>&sesskey=<?php echo sesskey() ?>"+datastring);
...
var API_1484_11 = new RSCORMapi1_3();

<?php
// pull in the debugging utilities
if (rscorm_debugging($scorm)) {
    include_once($CFG->dirroot.'/mod/rscorm/datamodels/debug.js.php');
    echo 'AppendToLog("Moodle RSCORM 1.3 API Loaded, Activity: '.$scorm->name.', SCO: '.$sco->identifier.'", 0);';
}
