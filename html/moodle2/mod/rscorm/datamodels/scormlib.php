...
function rscorm_get_resources($blocks) {
    $resources = array();
...
function rscorm_get_manifest($blocks, $scoes) {
    global $OUTPUT;
        ...
        foreach ($blocks as $block) {
            switch ($block['name']) {
                            ...
                            if ($metadata['name'] == 'SCHEMAVERSION') {
                                if (empty($scoes->version)) {
                                    if (isset($metadata['tagData']) && (preg_match("/^(1\.2)$|^(CAM )?(1\.3)$/", $metadata['tagData'], $matches))) {
                                        $scoes->version = 'RSCORM_'.$matches[count($matches)-1];
                                    } else {
                                        if (isset($metadata['tagData']) && (preg_match("/^2004 (3rd|4th) Edition$/", $metadata['tagData'], $matches))) {
                                            $scoes->version = 'RSCORM_1.3';
                                        } else {
                                            $scoes->version = 'RSCORM_1.2';
                                        }
                                    }
                                }
                            }
                ...
                case 'MANIFEST':
                    ...
                    $organization = '';
                    $resources = array();
                    $resources = rscorm_get_resources($block['children']);
                    $scoes = rscorm_get_manifest($block['children'], $scoes);
                    if (empty($scoes->elements) || count($scoes->elements) <= 0) {
                        foreach ($resources as $item => $resource) {
                            if (!empty($resource['HREF'])) {
                                ...
                                $sco->scormtype = $resource['ADLCP:RSCORMTYPE'];
                ...
                case 'ITEM':
                    ...
                    if (!isset($block['attrs']['IDENTIFIERREF'])) {
                        ...
                        if (!isset($resources[$idref])) {
                            ...
                            if (empty($resources[$idref]['ADLCP:RSCORMTYPE'])) {
                                $resources[$idref]['ADLCP:RSCORMTYPE'] = 'asset';
                            }
                            $scoes->elements[$manifest][$organization][$identifier]->scormtype = $resources[$idref]['ADLCP:RSCORMTYPE'];
                        }
                    }
    ...
    if (!empty($manifestresourcesnotfound)) {
        ...
        echo $OUTPUT->notification(get_string('invalidmanifestresource', 'rscorm').' '. implode(', ',$manifestresourcesnotfound));
    }
    return $scoes;
}

// MARSUPIAL ********** MODIFICAT - Change of function parameters
// 2012.12.11 @abertranb
function rscorm_parse_scorm($pkgdir, $scorm) {
	global $CFG, $DB, $USER;
...
// ********** ORIGINAL	
/*function rscorm_parse_scorm($scorm, $manifest) {
	global $CFG, $DB;
...
// ********** FI
    ...
    $scoes = new stdClass();
    $scoes->version = '';
    $scoes = rscorm_get_manifest($manifests, $scoes);
    if (count($scoes->elements) > 0) {
        $olditems = $DB->get_records('rscorm_scoes', array('scorm'=>$scorm->id));
        foreach ($scoes->elements as $manifest => $organizations) {
            foreach ($organizations as $organization => $items) {
                foreach ($items as $identifier => $item) {
                	
// MARSUPIAL ********** AFEGIT - Compare old item with new, and if something change update else do nothing
// 2012.12.10 @abertranb
                	$olditemid = rscorm_array_search('identifier',$item->identifier,$olditems);
                	if(!empty($olditems)){
...
// ********* FI                      
                     // This new db mngt will support all RSCORM future extensions
                	$newitem = new stdClass();
                    
// MARSUPIAL ********** MODIFICAT -> Added Marsupial Customize
// 2012.12.10 @abertranb
                	$update=false;
                	if (!$olditemid || ($olditemid && $olditem->scorm!=$scorm->id)){
                		$newitem->scorm = $scorm->id;
                		$update=true;
                	}
                	if (!$olditemid || ($olditemid && $olditem->manifest!=$manifest)){
                	...
                	if (!$olditemid || ($olditemid && isset($item->identifier) && $olditem->identifier!=$item->identifier)){
                		$newitem->identifier = addslashes_js($item->identifier);
                		$update=true;
                	}
                	...
                	if (!$olditemid || ($olditemid && isset($item->title) && $olditem->title!=$item->title)){
                		...
                		$newitem->title = addslashes($item->title);
                		// ********* ORIGINAL
                		//$newitem->title = addslashes_js($item->title);
                		// ********* FI
                		$update=true;
                	}
...
// MARSUPIAL ********** MODIFICAT - Compare old item with new, and if something change update else do nothing
// 2012.12.10 @abertranb
                    if(!$olditemid){
                    // Insert the new SCO, and retain the link between the old and new for later adjustment
                    $id = $DB->insert_record('rscorm_scoes', $newitem);
                    if (!empty($olditems) && ($olditemid = rscorm_array_search('identifier', $newitem->identifier, $olditems))) {
                        $olditems[$olditemid]->newid = $id;
                    }
                    }else if($update){
                    	$newitem->id=$id=$olditem->id;
                    	$DB->update_record('rscorm_scoes',$newitem);
                    }
                    else{
                    	$id=$olditemid;
                    }
                    if (isset($item->launch)&&$item->launch!=''){
                    	$scouser=new stdClass();
                    	if (!$scouserid=$DB->get_record('rscorm_scoes_users',array('scormid'=>$scorm->id,'scoid'=>$id,'userid'=>$USER->id))){
                    		$scouser->scormid=$scorm->id;
                    		$scouser->scoid=$id;
                    		$scouser->userid=$USER->id;
                    		$scouser->launch=$item->launch;
                    		$scouser->timecreated=time();
                    		$scouserid=$DB->insert_record('rscorm_scoes_users',$scouser);
                    	}else{
                    		$scouser->id=$scouserid->id;
                    		$scouser->launch=$item->launch;
                    		$scouser->timemodified=time();
                    		$DB->update_record('rscorm_scoes_users',$scouser);
                    	}
                    }
                    $standarddatas = array('parent', 'identifier', 'launch', 'scormtype', 'title');
// ********* ORIGINAL
...
// ********* FI
                    ...
                    if ($optionaldatas = rscorm_optionals_data($item, $standarddatas)) {
                        $data = new stdClass();
                        $data->scoid = $id;
                        foreach ($optionaldatas as $optionaldata) {
                            if (isset($item->$optionaldata)) {
                                ...
                                $dataid = $DB->insert_record('rscorm_scoes_data', $data);
                            }
                        }
                    }
                    ...
                    if (isset($item->sequencingrules)) {
                        foreach ($item->sequencingrules as $sequencingrule) {
                            ...
                            $ruleid = $DB->insert_record('rscorm_seq_ruleconds', $rule);
                            if (isset($sequencingrule->ruleconditions)) {
                                foreach ($sequencingrule->ruleconditions as $rulecondition) {
                                    ...
                                    $rulecondid = $DB->insert_record('rscorm_seq_rulecond', $rulecond);
                                }
                            }
                        }
                    }
                    ...
                    if (isset($item->rolluprules)) {
                        foreach ($item->rolluprules as $rolluprule) {
                            ...
                            $rollupruleid = $DB->insert_record('rscorm_seq_rolluprule', $rollup);
                            if (isset($rollup->conditions)) {
                                foreach ($rollup->conditions as $condition) {
                                    ...
                                    $conditionid = $DB->insert_record('rscorm_seq_rolluprulecond', $cond);
                                }
                            }
                        }
                    }
                    ...
                    if (isset($item->objectives)) {
                        foreach ($item->objectives as $objective) {
                            ...
                            $objectiveid = $DB->insert_record('rscorm_seq_objective', $obj);
                            if (isset($objective->mapinfos)) {
                                foreach ($objective->mapinfos as $objmapinfo) {
                                    ...
                                    $mapinfoid = $DB->insert_record('rscorm_seq_mapinfo', $mapinfo);
                                }
                            }
                        }
                    }
...
// MARSUPIAL ********** ELIMINAT - Taked out becouse is better to update than delete all and add it again each time
/*
        if (!empty($olditems)) {
            foreach ($olditems as $olditem) {
                $DB->delete_records('rscorm_scoes', array('id'=>$olditem->id));
                $DB->delete_records('rscorm_scoes_data', array('scoid'=>$olditem->id));
                if (isset($olditem->newid)) {
                    $DB->set_field('rscorm_scoes_track', 'scoid', $olditem->newid, array('scoid' => $olditem->id));
                }
                $DB->delete_records('rscorm_scoes_track', array('scoid'=>$olditem->id));
                $DB->delete_records('rscorm_seq_objective', array('scoid'=>$olditem->id));
                $DB->delete_records('rscorm_seq_mapinfo', array('scoid'=>$olditem->id));
                $DB->delete_records('rscorm_seq_ruleconds', array('scoid'=>$olditem->id));
                $DB->delete_records('rscorm_seq_rulecond', array('scoid'=>$olditem->id));
                $DB->delete_records('rscorm_seq_rolluprule', array('scoid'=>$olditem->id));
                $DB->delete_records('rscorm_seq_rolluprulecond', array('scoid'=>$olditem->id));
            }
        }*/
//********** FI
        if (empty($scoes->version)) {
        ...
        $DB->set_field('rscorm', 'version', $scoes->version, array('id'=>$scorm->id));
...
function rscorm_optionals_data($item, $standarddata) {
    $result = array();
...
function rscorm_is_leaf($sco) {
    global $DB;

    if ($DB->get_record('rscorm_scoes', array('scorm'=>$sco->scorm, 'parent'=>$sco->identifier))) {
        return false;
    }
    return true;
}

function rscorm_get_parent($sco) {
    global $DB;

    if ($sco->parent != '/') {
        if ($parent = $DB->get_record('rscorm_scoes', array('scorm'=>$sco->scorm, 'identifier'=>$sco->parent))) {
            return rscorm_get_sco($parent->id);
        }
    }
    return null;
}

function rscorm_get_children($sco) {
    global $DB;

    if ($children = $DB->get_records('rscorm_scoes', array('scorm'=>$sco->scorm, 'parent'=>$sco->identifier))) {//originally this said parent instead of childrean
        return $children;
    }
    return null;
}

function rscorm_get_available_children($sco) {  // TODO: undefined vars!!!
    global $DB;

    $res = $DB->get_record('rscorm_scoes_track', array('scoid'=>$scoid,
                                                     'userid'=>$userid,
...
function rscorm_get_available_descendent($descend = array(), $sco) {
    if ($sco == null) {
        return $descend;
    } else {
        $avchildren = rscorm_get_available_children($sco);
        foreach ($avchildren as $avchild) {
            array_push($descend, $avchild);
        }
        foreach ($avchildren as $avchild) {
            rscorm_get_available_descendent($descend, $avchild);
        }
    }
}

function rscorm_get_siblings($sco) {
    global $DB;

    if ($siblings = $DB->get_records('rscorm_scoes', array('scorm'=>$sco->scorm, 'parent'=>$sco->parent))) {
...
function rscorm_get_ancestors($sco) {
    $ancestors = array();
    $continue = true;
    while ($continue) {
        $ancestor = rscorm_get_parent($sco);
...
function rscorm_get_preorder($preorder=array(), $sco) {
    if ($sco != null) {
        array_push($preorder, $sco);
        $children = rscorm_get_children($sco);
        foreach ($children as $child) {
            rscorm_get_preorder($sco);
        }
    } else {
        return $preorder;
    }
...
function rscorm_find_common_ancestor($ancestors, $sco) {
    $pos = rscorm_array_search('identifier', $sco->parent, $ancestors);
    if ($sco->parent != '/') {
        if ($pos === false) {
            return rscorm_find_common_ancestor($ancestors, rscorm_get_parent($sco));
        }
    }
    return $pos;
}
