...
function getLoggingActive () {
    var loggingActive = getCookie('RSCORMLoggingActive');
    if (!loggingActive) {
        loggingActive = 'A';
    }
...
function setLoggingActive (flag) {
    new cookie("RSCORMLoggingActive", flag, 365, "/").set();
}
...
function toggleLog () {
    if (getLoggingActive() == "A") {
        ...
        setLoggingActive('N');
        logButton.innerHTML = '--><?php echo addslashes_js(get_string('rscormloggingoff', 'rscorm')); ?>';
    } else {
        setLoggingActive('A');
        AppendToLog("Moodle Logging Activated", 0);
        logButton.innerHTML = '<?php echo addslashes_js(get_string('rscormloggingon', 'rscorm')); ?>';
...
var debugRSCORMVersion = '<?php echo $scorm->version; ?>';
<?php
   $LMS_prefix = (rscorm_version_check($scorm->version, RSCORM_12) || empty($scorm->version)) ? 'LMS' : '';
   $LMS_api = (rscorm_version_check($scorm->version, RSCORM_12) || empty($scorm->version)) ? 'API' : 'API_1484_11';

   $LMS_elements = array();
   if (rscorm_version_check($scorm->version, RSCORM_12) || empty($scorm->version)) {
...
function UpdateLog(s) {
    var s1 = '<html><head><style>\n'
        ...
        + '          \' so API calls will fail - did navigate to another RSCORM package?\'); \n'
        + '    return false; \n'
        + '} \n'
        + ' \n'
        + 'var saveElement = ""; \n'
        ...
        + '<h1>RSCORM Debugging interface</h1>'
        + '<h2>RSCORM Version Detected: <?php echo $scorm->version; ?></h2>'
        + '<input type="hidden" id="mod-rscorm-logstate" name="mod-rscorm-logstate" value="A" \/>'
...
?>
        ...
        + '             name="commitButton">'
        + '            <input type="button"'
        + '             value="<?php echo rscorm_version_check($scorm->version, RSCORM_12) ? 'LMSFinish' : 'Terminate'; ?>()  "'
        + '             onclick="try<?php echo rscorm_version_check($scorm->version, RSCORM_12) ? 'LMSFinish' : 'LMSTerminate'; ?>();"'
        + '             id="finishButton"'
        + '             name="finishButton">'
        + '         </td>'
        + '    </tr>'
        ...
        + '   </table>'
        + '</form>'
        + '<h3 id="mod-rscorm-marker">RSCORM API Activity Log<\/h3>';
    ...
    if (logPopUpWindow != 'N' && !logPopUpWindow.closed) {
        ...
        logPopUpWindow = open( '', 'rscormlogpopupwindow', '' );
        popupdoc = logPopUpWindow.document;
        // Is logging active?
        var marker = safeGetElement(popupdoc, 'mod-rscorm-marker');
        if (marker) {
            popupdoc.body.innerHTML += s;
        } else {
            popupdoc.open();
            ...
            popupdoc.close();
            popupdoc.title = 'RSCORM API Activity Log';
            logPopUpWindow.focus();
        }
    }
...
function AppendToLog(s, rc) {
    ...
    if (s.match(/Commit|Loaded|Initialize|Terminate|Finish|Moodle RSCORM|Moodle Logging/)) {
        logRow++;
    }
}
...
logButton.id = 'mod-rscorm-log-toggle';
...
var content = safeGetElement(document, 'rscormpage');
