...
function rscorm_parse_aicc($scorm) {
    global $DB;

    if ($scorm->scormtype == RSCORM_TYPE_AICCURL) {
        return rscorm_aicc_generate_simple_sco($scorm);
    }
    if (!isset($scorm->cmid)) {
    ...
    $files = $fs->get_area_files($context->id, 'mod_rscorm', 'content', 0, 'sortorder, itemid, filepath, filename', false);
    ...
    $oldscoes = $DB->get_records('rscorm_scoes', array('scorm'=>$scorm->id));
    ...
    if (isset($courses)) {
        foreach ($courses as $course) {
            ...
            if ($ss = $DB->get_record('rscorm_scoes', array('scorm'=>$scorm->id,
                                                           'identifier'=>$sco->identifier))) {
                $id = $ss->id;
                $sco->id = $id;
                $DB->update_record('rscorm_scoes',$sco);
                unset($oldscoes[$id]);
            } else {
                $id = $DB->insert_record('rscorm_scoes', $sco);
            }
            ...
            if (isset($course->elements)) {
                foreach ($course->elements as $element) {
                    ...
                    if ($oldscoid = rscorm_array_search('identifier', $sco->identifier, $oldscoes)) {
                        ...
                        $DB->update_record('rscorm_scoes', $sco);
                        $id = $oldscoid;
                        $DB->delete_records('rscorm_scoes_data', array('scoid'=>$oldscoid));
                    ...
                    if (!empty($id)) {
                        ...
                        if (isset($element->web_launch)) {
                            ...
                            $dataid = $DB->insert_record('rscorm_scoes_data', $scodata);
                        }
    ...
    if (!empty($oldscoes)) {
        foreach ($oldscoes as $oldsco) {
            $DB->delete_records('rscorm_scoes', array('id'=>$oldsco->id));
            $DB->delete_records('rscorm_scoes_track', array('scoid'=>$oldsco->id));
        }
    }
 ...
 */
function rscorm_aicc_get_hacp_session($scormid) {
    ...
    $DB->insert_record('rscorm_aicc_session', $hacpsession);
 ...
 */
function rscorm_aicc_confirm_hacp_session($hacpsession) {
    global $DB;
    $cfg_scorm = get_config('rscorm');
    if (empty($cfg_scorm->allowaicchacp)) {
        return false;
    ...
    $hacpsession = $DB->get_record_select('rscorm_aicc_session', $sql, array($hacpsession, $time));
    if (!empty($hacpsession)) { //update timemodified as this is still an active session - resets the timeout.
        ...
        $DB->update_record('rscorm_aicc_session', $hacpsession);
    }
    return $hacpsession;
}
 ...
 */
function rscorm_aicc_generate_simple_sco($scorm) {
    global $DB;
    // find the old one
    $scos = $DB->get_records('rscorm_scoes', array('rscorm'=>$scorm->id));
    if (!empty($scos)) {
