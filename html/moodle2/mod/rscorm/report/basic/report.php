...
class rscorm_basic_report extends rscorm_default_report {
    /**
     * displays the full report
     * @param stdClass $rscorm full RSCORM object
    ...
    function display($scorm, $cm, $course, $download) {
        ...
        // detailed report
        $mform = new mod_rscorm_report_settings($PAGE->url, compact('currentgroup'));
        if ($fromform = $mform->get_data()) {
            $detailedrep = $fromform->detailedrep;
            $pagesize = $fromform->pagesize;
            set_user_preference('rscorm_report_detailed', $detailedrep);
            set_user_preference('rscorm_report_pagesize', $pagesize);
        } else {
            $detailedrep = get_user_preferences('rscorm_report_detailed', false);
        ...
        if ( !$nostudents ) {
            ...
            if ($detailedrep && $scoes = $DB->get_records('rscorm_scoes', array("scorm"=>$scorm->id), 'id')) {
            ...
            $select .= 'st.scormid AS scormid, st.attempt AS attempt, ' .
                    ...
                    //MARSUPIAL ************ MODIFICAT - To fix bug with SCORM report
