...
$sco = $DB->get_record('rscorm_scoes', array('id'=>$scoid), '*', MUST_EXIST);
$scorm = $DB->get_record('rscorm', array('id'=>$sco->scorm	), '*', MUST_EXIST);
$cm = get_coursemodule_from_instance('rscorm', $scorm->id, 0, false, MUST_EXIST);
...
if(!$nostudents) {
    ...
    $from = 'FROM {rscorm_scoes_track} st ';
    $where = ' WHERE st.userid ' .$usql. ' and st.scoid = ?';
    $attempts = $DB->get_records_sql($select.$from.$where, $params);

    foreach ($attempts as $attempt) {
        if ($trackdata = rscorm_get_tracks($scoid, $attempt->userid, $attempt->attempt)) {
...
$bandlabels[] = get_string('invaliddata', 'rscormreport_graphs');
for ($i = 1; $i <= $bands-1; $i++) {
...
$line->parameter['y_label_left'] = get_string('participants', 'rscormreport_graphs');
$line->parameter['x_label'] = get_string('percent', 'rscormreport_graphs');
