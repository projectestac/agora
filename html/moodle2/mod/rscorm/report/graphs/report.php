...
class rscorm_graphs_report extends rscorm_default_report {
    /**
    ...
    function display($scorm, $cm, $course, $download) {
        ...
        if ($scoes = $DB->get_records('rscorm_scoes', array("scorm"=>$scorm->id), 'id')) {
            foreach ($scoes as $sco) {
                if ($sco->launch != '') {
                    $imageurl = new moodle_url('/mod/rscorm/report/graphs/graph.php',
                            array('scoid' => $sco->id));
                    $graphname = $sco->title;
