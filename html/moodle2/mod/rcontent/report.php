...
if (!isset($action) || ($action != "update" && $action != "saveupdate")) {
	if (empty($noheader)) {
        ...
        if (! $bookname = $DB->get_field('rcommon_books', 'name', array('id' => $rcontent->bookid))) {
		...
		if (empty($c)) {
	        if (empty($b)) {
	        	...
	        	if (!$unitname = $DB->get_field('rcommon_books_units', 'name', array('bookid' => $rcontent->bookid, 'id' => $b))) {
			...
			if (!$activityname = $DB->get_field('rcommon_books_activities', array('bookid' => $rcontent->bookid, 'unitid' => $b, 'id' => $c))) {
