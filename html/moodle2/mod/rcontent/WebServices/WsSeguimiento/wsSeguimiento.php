...
// ************ FI
                ...
                if ($unidad == false)
                {
                    ...
                    //obtiene el registro            
                    $unidad = $DB->get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books_units where id = ".$unid, array(), IGNORE_MULTIPLE);
...
// 2012.03.01 @mmartinez 
                	...
                	if ($actividad == false) {
                    	$actividad = $DB->get_record_sql("SELECT id FROM {$CFG->prefix}rcommon_books_activities WHERE unitid = ".$unidad->id." AND code = '".$ResultExt->idActividad."'", array(), IGNORE_MULTIPLE);
                	}
...
function valid_unit_activity($ResultExt, &$unidadid, &$actividadid, &$err)
{
    ...
    try
    {
        ...
        //search rcontent
        $rcontent = $DB->get_record_sql("SELECT * FROM {$CFG->prefix}rcontent where id = ".$ResultExt->idContenidoLMS, array(), IGNORE_MULTIPLE);

        //search book
        if ($book = $DB->get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books where id = {$rcontent->bookid}", array(), IGNORE_MULTIPLE))
        {
...
function UserAuthentication($post_data, $ResultExt)
{
    ...
    try
    {
        ...
        {
            if ($rcontent = $DB->get_record_sql("SELECT * FROM {$CFG->prefix}rcontent where id = ".$ResultExt->idContenidoLMS, array(), IGNORE_MULTIPLE))
            {
                if ($book = $DB->get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_books where id = {$rcontent->bookid}", array(), IGNORE_MULTIPLE))
                {
...
//********** MODIFICAT MARSUPIAL - changed validation table to rcommon_publisher
                        if ($track_credent = $DB->get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_publisher where id = ".$book->publisherid, array(), IGNORE_MULTIPLE))
//**********
                        {
                            $keys = array("Envelope", "Header", "WSEAuthenticateHeader", "User", "value");
