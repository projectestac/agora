...
/**
 * Strings for component 'admin', language 'ko', branch 'MOODLE_24_STABLE'
 *
 * @package   admin
...
$string['bookmarkadded'] = '북마크 추가됨';
$string['bookmarkalreadyexists'] = '이 페이지는 이미 북마크되어 있습니다.';
$string['bookmarkdeleted'] = '북마크주소 삭제됨';
$string['bookmarkthispage'] = '이 페이지를 북마크함';
...
$string['configallowthemechangeonurl'] = '활성화되면, 무들의 주소  <br />?theme=themename  (예: mymoodlesite.com/?theme=afterburner ) 혹은 무들 내부 주소 <br />&theme=themename  (예: mymoodlesite.com/course/view.php?id=2&theme=afterburner ) 를 추가해서 테마를 바꿀 수 있습니다.';
...
$string['configdefaulthomepage'] = '로그인 사용자를 위한 기본 홈페이지를 결정합니다.';
...
$string['configdefaultuserroleid'] = '로그인한 모든 사용자들은 부여된 역할에 추가하여 이곳에서 지정한 역할의 능력을 부여받게됩니다. 기본 역할은 인증된 사용자입니다. 이것은 능력을 제한하지 않는한 다른 역할과 상충되지 않으며, 단지 모든 사용자가 강좌 수준에서 부여할 수 없는 능력(포럼의 게시 및 일정 관리 등)을 갖도록 합니다.';
...
$string['configenableajax'] = '이 설정은 사이트 전역에서  AJAX 사용을 통제합니다. AJAX 기능은 드래그드롭과 같은 기능에 필요합니다.';
...
$string['configfullnamedisplay'] = '이것은 전체 이름이 표시될때 어떻게 표시될지를 정의합니다. 대부분의 한 언어만 쓰는 사이트에서는 기본값인 "이름+성"으로도 충분하지만, 성을 감추거나, 현재 사용중인 언어팩에서 결정하도록 할 수 있습니다. (언어마다 다른 관습이 있습니다)';
...
$string['configlangmenu'] = '홈페이지와 로그인페이지 등에서 일반적인 목적으로 언어 메뉴를 보이게 할 것인지 말 것인지를 설정하십시요. 이 설정은 사용자들이 개인정보에서 선호하는 언어를 설정하는 능력에는 영향을 미치지 않습니다.';
...
$string['confignavshowallcourses'] = '활성화되면 등록한 모든 강좌들을 볼 수 있습니다. 비활성화되면 내강좌에 있는 강좌들만 볼 수 있습니다. 사용자가 로그인을 하지 않았거나 등록하지 않은 경우 보여지는 강좌의 수는 "강좌한계(navcourselimit)" 설정에 의해 제한됩니다.';
...
$string['configrecaptchaprivatekey'] = '무들 서버와 recaptcha 서버 사이에 통신용으로 쓰이는 문자열.  http://www.google.com/recaptcha에 접속하여 이 사이트에 쓸 문자열을 받으십시요.';
$string['configrecaptchapublickey'] = '등록 양식에 reCAPTCHA 요소를 표시하기 위해 사용될 문자열.  http://www.google.com/recaptcha에서 생성됩니다.';
...
$string['configsitemaxcategorydepthhelp'] = '이것은 보여지는 하위 범주의 최대 깊이를 지정합니다';
...
$string['configuseexternalyui'] = '로컬 파일을 사용하는 대신 Yahoo 서버에서 온라인으로 제공하는 파일을 사용합니다. 주의: 인터넷 연결이 필요합니다. 그렇지 않으면 AJAX가 동작하지 않을 것입니다. 이 설정은 https 를 사용하는 사이트에서는 부합되지 않습니다.';
...
$string['defaulthomepage'] = '사용자 기본 홈페이지';
...
$string['nobookmarksforuser'] = '북마크된 주소가 없음';
...
$string['nonexistentbookmark'] = '요청한 북마크를 찾을 수 없습니다.';
...
$string['numcoursesincombo'] = '콤보 목록에서 최대 강좌 수';
$string['numcoursesincombo_help'] = '콤보 목록은 강좌의 수가 많을때는 잘 작동하지 않습니다. 사이트에 있는 총 강좌의 수가 이 설정값보다 크면 초기화면에 모든 강좌를 보여주는 대신 전용 강좌 목록에 대한 링크가 보여질 것입니다.';
...
$string['profiledeletefield'] = '항목 \'{$a}\'  삭제';
...
$string['themeresetcaches'] = '데마 캐시 청소';
...
$string['unbookmarkthispage'] = '이 페이지 북마크 해제';
...
$string['upgradepluginsfirst'] = '사용가능한 업데이트를 수동으로 다운받아  설치';
...
$string['upgradestart'] = '지금 무들 데이터베이스 업그레이드';
