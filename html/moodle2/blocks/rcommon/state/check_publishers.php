<?php
    ...
    foreach ($publisher_books as $pub){
        try{
        	$options = array('trace' => 1, 'connection_timeout' => 120, 'exceptions' => true);
            ...
            $publishers_check[$pub->id]['code'] = 0;
            $publishers_check[$pub->id]['description'] = $e->getMessage();
        } catch (SoapFault $e){
            $publishers_check[$pub->id]['code'] = 0;
            $publishers_check[$pub->id]['description'] = $e->getMessage();
        }
    ...
    foreach ($publisher_books as $publisher){
		if($publishers_check[$publisher->id]['code'] != '1'){
			$result .= '<li><span style="color:red">'.$publisher->name.' ('.$publisher->books.') - '.'<span style="font-size:small">'.get_string('bad_connection', 'block_rcommon').': '.$publishers_check[$publisher->id]['description'].'</span></span></li>';
		}else{
			$result .= '<li><span style="color:green">'.$publisher->name.' ('.$publisher->books.') - '.'<span style="font-size:small">'.get_string('good_connection', 'block_rcommon').'</span></span></li>';
        }
    }
    $result .= '</ul>';
