...
// 2011.10.24 @mmartinez
	                            $instance->name = rcommond_findarrayvalue($li, $keys);
// *********** ORIGINAL
                                //$instance->name = textlib::substr("'", "''", rcommond_findarrayvalue($li, $keys));
// *********** FI
//MARSUPIAL ********** AFEGIT -> Check that book name isn't large than 255 caracters
//2011.06.02 @mmartinez
                                if (textlib::strlen($instance->name) > 255) {
                                	$instance->name = textlib::substr($instance->name, 0, 255);
                                }
//********* FI
...
// MARSUPIAL ********** MODIFICAT -> Addslashes
// 2011.10.24 @mmartinez
	                            $instance->summary = rcommond_findarrayvalue($li, $keys);
// *********** ORIGINAL
                                //$instance->summary = textlib::substr("'", "''", rcommond_findarrayvalue($li, $keys));
// *********** FI
//MARSUPIAL ********** AFEGIT -> Check that book summary isn't large than 1024 caracters
//2011.06.02 @mmartinez
                                if (textlib::strlen($instance->summary) > 1024){
                                	$instance->summary = textlib::substr($instance->summary, 0, 1024);
                                }
//********* FI
	                            //$instance->format = str_replace("'", "''", $li["formato"]["value"]);
	                            $keys = array("formato", "value");
	                            $instance->format = textlib::strtolower(str_replace("'", "''", rcommond_findarrayvalue($li, $keys)));
...
//**********
	                            ...
	                            if ($levelid = $DB->get_field('rcommon_level','id', array('code'=>$nivel_libro))) {
	...
	//**********
	                            ...
	                            foreach($obligatoryarray as $value) {
	                            	if($instance->$value=='') {
	                            		echo "KO! -- <span style='color: red;'>Required parameter <b>".$value."</b> not found!</span>";
	                            ...
	                            if (!$regbookid = $DB->get_field('rcommon_books','id',array('isbn'=>$cod_isbn))) {
	                                $instance->timecreated = time();
	                                $bookid = $DB->insert_record('rcommon_books', $instance);
	                                echo "OK";
	                            } else {
	                                $instance->id = $regbookid;
	                                $bookid = $regbookid;
...
// MARSUPIAL ********** MODIFICAT -> Addslashes
// 2011.10.24 @mmartinez
	                                    $instance->name = rcommond_findarrayvalue($un, $keys);
// ********** ORIGINAL
                                        //$instance->name = textlib::substr("'", "''", rcommond_findarrayvalue($un, $keys));
// ********** FI
//MARSUPIAL ********** AFEGIT -> Check that unit name isn't large than 255 caracters
//2011.06.02 @mmartinez
		                                if (textlib::strlen($instance->name) > 200){
		                                	$instance->name = textlib::substr($instance->name, 0, 200);
		                                }
//********* FI
...
// MARSUPIAL ********** MODIFICAT -> Addslashes
// 2011.10.24 @mmartinez
	                                    $instance->summary = rcommond_findarrayvalue($un, $keys);
// ********** ORIGINAL
                                        //$instance->summary = textlib::substr("'", "''", rcommond_findarrayvalue($un, $keys));
//********* FI
	                                    //
...
//********** FI
	                                    ...
	                                    if (!$regunitid = $DB->get_field('rcommon_books_units','id', array('bookid'=>$bookid, 'code'=>$instance->code))) {
...
// *********** ORIGINAL
                                                //$instance->name = textlib::substr("'", "''", rcommond_findarrayvalue($act, array("titulo", "value")));
// *********** FI
...
// *********** ORIGINAL
                                                //$instance->summary = textlib::substr("'", "''", rcommond_findarrayvalue($act, array("titulo", "value")));
//********* FI
...
//********* FI
                                                ...
                                                $select_params = array('bookid'=>$bookid, 'unitid'=>$unitid, 'code'=>$instance->code);
	                                            //$select = $select." AND code='".$act["id"]["value"]."'";

	                                            if (!$regactivid = $DB->get_field('rcommon_books_activities','id', $select_params)) {
	                                                $instance->timecreated = time();
	                                                //echo "<br><br>"; print_r($instance); echo "<br><br>";
	                                                $activid = $DB->insert_record('rcommon_books_activities', $instance);
	                                            } else {
	                                                $instance->id = $regactivid;
	                                                $activid = $regactivid;
