...
// 2012.09.03 @mmartinez
	...
	// ********** FI
                ...
                if (($usr_creden = get_record_sql("SELECT * FROM {$CFG->prefix}rcommon_user_credentials WHERE isbn='".$book->isbn."' AND euserid='".$USER->id."'",true)) == false){
                    error(get_string('nocredentials','block_rcommon'));
                }
            }else{*/
// **************** FI
...
//*********** END
    ...
    //    elseif (($auth_creden = get_record('rcommon_wsauth_credentials', array('wsurl' => $usr_creden->urlwsauthentication))) == false){
    //        error('wsurl not found.');
    //    }
        elseif ($data->unitid != 0 && ($unit = $DB->get_record('rcommon_books_units', array('id' => $data->unitid))) == false){
...
// 2014.01.13 @naseq
        ...
        try {
            ...
            echo '<script type="text/javascript">window.alert("' . get_string('bad_wsdl_connection_alert', 'block_rcommon') . '"); history.go(-1);</script>';
...
// ************** FI
            ...
            if($urlok && $showurl)
            {
                $msg='<br><br>'.@get_string('urlmoreinfo','block_rcommon',$response->AutenticarUsuarioContenidoResult->URL);
