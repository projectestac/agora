...
$url = new moodle_url('/blocks/my_books/manageKey.php'); // Base URL
$PAGE->set_url($url);
...
switch($action){
	case 'edit':
		...
		$book       = $DB->get_record('rcommon_books', array('isbn' => $credential->isbn));
	...
	case 'doEdit':
		...
		$credential = $DB->get_record_sql ("SELECT b.id AS bookid, cred.euserid as userid, cred.credentials, b.name, b.isbn FROM {rcommon_user_credentials} cred LEFT JOIN {rcommon_books} b ON cred.isbn = b.isbn WHERE cred.id = '{$id}'");
	...
	case 'unassing':
			...
			// MARSUPIAL *************** MODIFICAT -> Deprected
			// 2012.12.14 @abertranb
			if ($DB->execute("UPDATE {rcommon_user_credentials} SET euserid = '0', timemodified = '" . time() . "' WHERE id IN ({$id})" )){
			// *************** ORIGINAL
	...
	default:
		//load data form db
		$sql = "SELECT ruc.id, rp.name as ed_name, rb.name as bk_name, ruc.isbn, ruc.credentials FROM {rcommon_user_credentials} ruc LEFT JOIN {rcommon_books} rb ON ruc.isbn = rb.isbn LEFT JOIN {$CFG->prefix}rcommon_publisher rp ON rb.publisherid = rp.id WHERE ruc.euserid = '{$USER->id}' ORDER BY ed_name, bk_name";
