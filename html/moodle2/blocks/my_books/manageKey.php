...
$ids       = optional_param_array('ids', array(), PARAM_RAW);
...
switch($action){		
	...
	case 'doEdit':
		...
		$credential = $DB->get_record_sql ("SELECT b.id AS bookid, cred.euserid as userid, cred.credentials, b.name, b.isbn FROM {$CFG->prefix}rcommon_user_credentials cred LEFT JOIN {$CFG->prefix}rcommon_books b ON cred.isbn = b.isbn WHERE cred.id = '{$id}'");
	...
	case 'unassing':
			...
			if ($DB->execute("UPDATE {$CFG->prefix}rcommon_user_credentials SET euserid = '0', timemodified = '" . time() . "' WHERE id IN ({$id})" )){
			// *************** ORIGINAL
