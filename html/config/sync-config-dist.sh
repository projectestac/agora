#!/bin/bash

# DEV
PHP_PATH=/usr/bin/php

# CMO
#PHP_PATH="/opt/xtk/php5/bin/php -c /opt/xtk/php5/lib/php.ini"

# FMO AGORA INT
#PHP_PATH="/opt/rh/php54/root/usr/bin/php -c /serveis/conf/int/1051/apache/php.ini"

# FMO AGORA PRE
#PHP_PATH="/opt/rh/php54/root/usr/bin/php -c /serveis/conf/pre/1051/apache/php.ini"

# FMO AGORA PRO
#PHP_PATH="/opt/rh/php54/root/usr/bin/php -c /serveis/conf/pro/1051/apache/php.ini"

# FMO AGORA-EOI INT
#PHP_PATH="/opt/rh/php54/root/usr/bin/php -c /serveis/conf/int/1143/apache/php.ini"

# FMO AGORA-EOI PRE
#PHP_PATH="/opt/rh/php54/root/usr/bin/php -c /serveis/conf/pre/1143/apache/php.ini"

# FMO AGORA-EOI PRO
#PHP_PATH="/opt/rh/php54/root/usr/bin/php -c /serveis/conf/pro/11143/apache/php.ini"
